...
InterpolationTrussLobatto::InterpolationTrussLobatto(int order)
    ...
    , mShapeFunctionMemo([this](NaturalCoords x) { return ShapeFunctions(x[0], mNodes); })
    , mShapeFunctionDerivativesMemo([this](NaturalCoords x) { return DerivativeShapeFunctions(x[0], mNodes); })
{
}

InterpolationTrussLobatto::InterpolationTrussLobatto(const InterpolationTrussLobatto& other)
    : mNodes(other.mNodes)
    , mShapeFunctionMemo([this](NaturalCoords x) { return ShapeFunctions(x[0], mNodes); })
    , mShapeFunctionDerivativesMemo([this](NaturalCoords x) { return DerivativeShapeFunctions(x[0], mNodes); })
{
