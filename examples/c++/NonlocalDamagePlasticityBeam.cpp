    ...
    try
    {
        ...
        for (int theNodeY = 0; theNodeY < NumNodeY; theNodeY++, posY += deltaY)
        {
            ...
            for (int theNodeX = 0; theNodeX < NumNodeX; theNodeX++, posX += deltaX)
            {
                Coordinates[0] = posX;
                Coordinates(1) = posY;
                int theNode = myStructure.NodeCreate(Coordinates);
                if (posX == 0)
                {
                    if (posY == 0)
        ...
        for (int theElementY = 0; theElementY < NumElementY; theElementY++)
        {
            for (int theElementX = 0; theElementX < NumElementX; theElementX++)
            {
                ...
                myStructure.ElementCreate(myInterpolationType, Incidence,NuTo::ElementData::eElementDataType::CONSTITUTIVELAWIPNONLOCAL,NuTo::IpData::eIpDataType::STATICDATANONLOCAL);
            }
        }
        ...
        while (curDisp < maxDisp)
        {
            ...
            if (numNewtonIterations < MAXNUMNEWTONITERATIONS)
            {
                ...
                if (PRINTRESULT)
                {
                    ...
                    Stress.Trans().Info();
                    std::cout << "number of Newton iterations " << numNewtonIterations << ", delta disp " << deltaMaxDisp << ", maxResidual " << maxResidual << ", normResidual " << normResidual
                            << std::endl;
                }
                ...
                // visualize results
#ifdef ENABLE_VISUALIZE
                myStructure.ExportVtkDataFileElements("NonlocalDamagePlasticityBeamView.vtk");
#endif
