...
BOOST_AUTO_TEST_CASE(run2d)
{
    ...
    const boost::filesystem::path resultDir(boost::filesystem::initial_path().string() +
                                            "/results_ConstraintsNodeToElement2d/");
    const boost::filesystem::path meshFilePathMatrix(boost::filesystem::initial_path().string() +
                                                     "/meshes/ConstraintsNodeToElementMatrixMesh2d.msh");
    const boost::filesystem::path meshFilePathFiber(boost::filesystem::initial_path().string() +
                                                    "/meshes/ConstraintsNodeToElementFiberMesh2d.msh");
    ...
    int matrixMaterial =
            s.ConstitutiveLawCreate(NuTo::Constitutive::eConstitutiveType::LINEAR_ELASTIC_ENGINEERING_STRESS);
    s.ConstitutiveLawSetParameterDouble(matrixMaterial, NuTo::Constitutive::eConstitutiveParameter::YOUNGS_MODULUS,
                                        Parameters::mMatrixYoungsModulus);
    s.ConstitutiveLawSetParameterDouble(matrixMaterial, NuTo::Constitutive::eConstitutiveParameter::POISSONS_RATIO,
                                        Parameters::mMatrixPoissonsRatio);

    //        int matrixMaterial =
    //        s.ConstitutiveLawCreate(NuTo::Constitutive::eConstitutiveType::GRADIENT_DAMAGE_ENGINEERING_STRESS);
    //        s.ConstitutiveLawSetParameterDouble(matrixMaterial,
    ...
    //        s.ConstitutiveLawSetParameterDouble(matrixMaterial,
    //        NuTo::Constitutive::eConstitutiveParameter::COMPRESSIVE_STRENGTH, Parameters::mMatrixCompressiveStrength);
    //        s.ConstitutiveLawSetParameterDouble(matrixMaterial,
    ...
    int fibreMaterial =
            s.ConstitutiveLawCreate(NuTo::Constitutive::eConstitutiveType::LINEAR_ELASTIC_ENGINEERING_STRESS);
    s.ConstitutiveLawSetParameterDouble(fibreMaterial, NuTo::Constitutive::eConstitutiveParameter::YOUNGS_MODULUS,
                                        Parameters::mFibreYoungsModulus);
    s.ConstitutiveLawSetParameterDouble(fibreMaterial, NuTo::Constitutive::eConstitutiveParameter::POISSONS_RATIO,
    ...
    s.InterpolationTypeAdd(matrixInterpolationType, NuTo::Node::eDof::COORDINATES,
                           NuTo::Interpolation::eTypeOrder::EQUIDISTANT1);
    s.InterpolationTypeAdd(matrixInterpolationType, NuTo::Node::eDof::DISPLACEMENTS,
                           NuTo::Interpolation::eTypeOrder::EQUIDISTANT1);
    //        s.InterpolationTypeAdd(matrixInterpolationType, NuTo::Node::eDof::NONLOCALEQSTRAIN,
    //        NuTo::Interpolation::eTypeOrder::EQUIDISTANT1);


    int fibreInterpolationType = s.InterpolationTypeCreate(NuTo::Interpolation::eShapeType::TRUSSXD);
    s.InterpolationTypeAdd(fibreInterpolationType, NuTo::Node::eDof::COORDINATES,
                           NuTo::Interpolation::eTypeOrder::EQUIDISTANT1);
    s.InterpolationTypeAdd(fibreInterpolationType, NuTo::Node::eDof::DISPLACEMENTS,
                           NuTo::Interpolation::eTypeOrder::EQUIDISTANT1);
    ...
    s.Constraints().Add(NuTo::Node::eDof::DISPLACEMENTS,
                        NuTo::Constraint::Component(*nodeGroup, {NuTo::eDirection::X}));
    ...
    s.Constraints().Add(NuTo::Node::eDof::DISPLACEMENTS,
                        NuTo::Constraint::Component(*nodeRight, {NuTo::eDirection::Y}));
    ...
    s.Constraints().Add(
            ...
            NuTo::Constraint::Component(*nodeGroupBC, {NuTo::eDirection::X},
                                        NuTo::Constraint::RhsRamp(Parameters::mSimulationTime, Parameters::mLoad)));
    ...
    auto dispInMatrix =
            elementPtrMatrix->InterpolateDofGlobal(Eigen::Vector2d(0.5, 0.25), NuTo::Node::eDof::DISPLACEMENTS);
    auto coordsInMatrix =
            elementPtrMatrix->InterpolateDofGlobal(Eigen::Vector2d(0.5, 0.25), NuTo::Node::eDof::COORDINATES);
...
BOOST_AUTO_TEST_CASE(run3d)
{
    ...
    const boost::filesystem::path resultDir(boost::filesystem::initial_path().string() +
                                            "/results_ConstraintsNodeToElement3d/");
    const boost::filesystem::path meshFilePathMatrix(boost::filesystem::initial_path().string() +
                                                     "/meshes/ConstraintsNodeToElementMatrixMesh3d.msh");
    const boost::filesystem::path meshFilePathFiber(boost::filesystem::initial_path().string() +
                                                    "/meshes/ConstraintsNodeToElementFiberMesh3d.msh");
    ...
    s.ConstitutiveLawSetParameterDouble(matrixMaterial, NuTo::Constitutive::eConstitutiveParameter::POISSONS_RATIO,
                                        Parameters::mMatrixPoissonsRatio);
    s.ConstitutiveLawSetParameterDouble(matrixMaterial, NuTo::Constitutive::eConstitutiveParameter::TENSILE_STRENGTH,
    ...
    s.ConstitutiveLawSetParameterDouble(matrixMaterial, NuTo::Constitutive::eConstitutiveParameter::NONLOCAL_RADIUS,
                                        Parameters::mMatrixNonlocalRadius);
    s.ConstitutiveLawSetDamageLaw(matrixMaterial,
                                  NuTo::Constitutive::DamageLawExponential::Create(
                                          Parameters::mMatrixTensileStrength / Parameters::mMatrixYoungsModulus,
                                          Parameters::mMatrixTensileStrength / Parameters::mMatrixFractureEnergy));
    ...
    s.Constraints().Add(NuTo::Node::eDof::DISPLACEMENTS,
                        NuTo::Constraint::Component(groupNodeBCLeft,
                                                    {NuTo::eDirection::X, NuTo::eDirection::Y, NuTo::eDirection::Z}));

    const auto& nodeRight = s.NodeGetAtCoordinate(Eigen::Vector3d(10, 0, 0));
    s.Constraints().Add(NuTo::Node::eDof::DISPLACEMENTS,
                        NuTo::Constraint::Component(nodeRight, {NuTo::eDirection::Y, NuTo::eDirection::Z}));
    ...
    for (int iDomain = 0; iDomain < numSearchDomains; ++iDomain)
    {
        ...
        s.GroupAddNodeFromElementGroupCoordinateRange(groupMatrixNodes, groupIdMatrix, 0, iDomain * deltaLength,
                                                      (iDomain + 1) * deltaLength);
        ...
        s.GroupAddNodeFromElementGroupCoordinateRange(groupConstraintNodes, groupIdFiber, 0, iDomain * deltaLength,
                                                      (iDomain + 1) * deltaLength);

        for (int nodeId : s.GroupGetMemberIds(groupConstraintNodes))
            s.ConstraintLinearEquationNodeToElementCreate(nodeId, groupMatrixElements, NuTo::Node::eDof::DISPLACEMENTS);
    }
