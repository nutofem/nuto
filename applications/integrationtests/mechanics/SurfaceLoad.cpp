...
{
    auto extLoadVector = rStructure.BuildGlobalExternalLoadVector().J[NuTo::Node::eDof::DISPLACEMENTS];

    int dimension = rStructure.GetDimension();
    int numNodes = rStructure.GetNumNodes();
    if (extLoadVector.rows() != numNodes*dimension)
        throw NuTo::Exception("[SurfaceLoad::CheckHydrostaticPressure] F_ext.GetNumRows() != dimension * numNodes. Maybe you (mistakenly) applied constraints.");
    ...
    if (extLoadVector.cwiseAbs().maxCoeff() < 1.e-6)
        throw NuTo::Exception("[SurfaceLoad::CheckHydrostaticPressure] No external load at all!");
    ...
    if (relativeError > 1.e-8)
    {
        ...
        throw NuTo::Exception("[SurfaceLoad::CheckHydrostaticPressure] The resulting forces should cancel out. But they don't.");
    }
}
...
void HydrostaticPressureTriangle2D(NuTo::Interpolation::eTypeOrder rInterpolationDisp)
{
    ...
    for (int iSurface = 0; iSurface < 3; ++iSurface)
        myStructure.LoadSurfacePressureCreate2D(elementGroup, surfaces[iSurface], pressure);
    ...
    std::cout << "[SurfaceLoad::HydrostaticPressureTriangle2D] " +
                         NuTo::Interpolation::TypeOrderToString(rInterpolationDisp) + " done."
              << std::endl;
}
...
void HydrostaticPressureQuad2D(NuTo::Interpolation::eTypeOrder rInterpolationDisp)
{
    ...
    CheckHydrostaticPressure(myStructure);
    std::cout << "[SurfaceLoad::HydrostaticPressureQuad2D] " +
                         NuTo::Interpolation::TypeOrderToString(rInterpolationDisp) + " done."
              << std::endl;
}
...
void HydrostaticPressureTetrahedron3D(NuTo::Interpolation::eTypeOrder rInterpolationDisp)
{
    ...
    nodeCoords << 0, 2, 0, 0, 1, 0, 3, 2, 0, 0, 0, 4;

    Eigen::Matrix3d rotationMatrix(Eigen::AngleAxisd(M_PI / 3., Eigen::Vector3d::UnitX()) *
    ...
    for (int iSurface = 0; iSurface < 4; ++iSurface)
        myStructure.LoadSurfacePressureCreate3D(elementGroup, surfaces[iSurface], pressure);

    CheckHydrostaticPressure(myStructure);
    std::cout << "[SurfaceLoad::HydrostaticPressureTetrahedron3D] " +
                         NuTo::Interpolation::TypeOrderToString(rInterpolationDisp) + " done."
              << std::endl;
}
...
void HydrostaticPressureBrick3D(NuTo::Interpolation::eTypeOrder rInterpolationDisp)
{
    ...
    nodeCoords << 1, 3, 3, 0, 1, 3, 3, 0, 0, 0, 4, 4, 0, 0, 4, 4, 1, 0, 1, 0, 5, 5, 5, 5;
    ...
    CheckHydrostaticPressure(myStructure);
    std::cout << "[SurfaceLoad::HydrostaticPressureBrick3D] " +
                         NuTo::Interpolation::TypeOrderToString(rInterpolationDisp) + " done."
              << std::endl;
}
...
void CheckSurfaceLoad(NuTo::Structure& rStructure, const Eigen::VectorXd& rLoad, double rSurfaceArea)
{
    ...
    int numNodes = rStructure.GetNumNodes();
    if (extLoadVector.rows() != numNodes*dimension)
        throw NuTo::Exception("[SurfaceLoad::CheckSurfaceLoad] F_ext.GetNumRows() != dimension * numNodes. Maybe you (mistakenly) applied constraints.");
    ...
    if (relativeError > 1.e-6)
    {
        ...
        std::cout << "Total external load vector:" << std::endl;
        std::cout << extLoadVector << std::endl;
        throw NuTo::Exception("[SurfaceLoad::CheckSurfaceLoad] Wrong external load calculation!");
    }
}
...
void SurfaceLoadTriangle2D(NuTo::Interpolation::eTypeOrder rInterpolationDisp)
{
    ...
    myStructure.LoadSurfaceConstDirectionCreate2D(elementGroup, s, load);

    CheckSurfaceLoad(myStructure, load, surfaceArea);
    std::cout << "[SurfaceLoad::SurfaceLoadTriangle2D] " + NuTo::Interpolation::TypeOrderToString(rInterpolationDisp) +
                         " done."
              << std::endl;
}
...
void SurfaceLoadQuad2D(NuTo::Interpolation::eTypeOrder rInterpolationDisp)
{
    ...
    nodeCoords << 0, lx, 0, -lx, -ly, 0, ly, 0;
    ...
    CheckSurfaceLoad(myStructure, load, surfaceArea);
    std::cout << "[SurfaceLoad::SurfaceLoadQuad2D] " + NuTo::Interpolation::TypeOrderToString(rInterpolationDisp) +
                         " done."
              << std::endl;
}
...
void SurfaceLoadQuad2DIGA(NuTo::Interpolation::eTypeOrder rInterpolationDisp)
{
    ...
    myStructure.InterpolationTypeAdd(interpolationType, NuTo::Node::eDof::COORDINATES,
                                     NuTo::Interpolation::eTypeOrder::SPLINE, degree, vecKnots, weights);
    ...
    myStructure.ElementCreate(interpolationType, elementIncidence, elementKnots, elementKnotIDs);
...
void SurfaceLoadTetrahedron3D(NuTo::Interpolation::eTypeOrder rInterpolationDisp)
{
    ...
    nodeCoords << 0, lx, 0, 0, 0, 0, ly, 0, 0, 0, 0, lz;

    // rotate randomly
    Eigen::Matrix3d rotationMatrix(Eigen::AngleAxisd(M_PI / 2.9, Eigen::Vector3d::UnitX()) *
    ...
    myStructure.LoadSurfaceConstDirectionCreate3D(elementGroup, s, load);

    CheckSurfaceLoad(myStructure, load, surfaceArea);
    std::cout << "[SurfaceLoad::SurfaceLoadTetrahedron3D] " +
                         NuTo::Interpolation::TypeOrderToString(rInterpolationDisp) + " done."
              << std::endl;
}
...
void SurfaceLoadBrick3D(NuTo::Interpolation::eTypeOrder rInterpolationDisp)
{
    ...
    nodeCoords << 0, lx, lx, 0, 0, lx, lx, 0, 0, 0, ly, ly, 0, 0, ly, ly, 0, 0, 0, 0, lz, lz, lz, lz;
    ...
    CheckSurfaceLoad(myStructure, load, surfaceArea);
    std::cout << "[SurfaceLoad::SurfaceLoadBrick3D] " + NuTo::Interpolation::TypeOrderToString(rInterpolationDisp) +
                         " done."
              << std::endl;
}
