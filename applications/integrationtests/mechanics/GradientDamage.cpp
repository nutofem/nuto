...
BOOST_AUTO_TEST_CASE(Integrand)
{
    ...
    MeshFem mesh = UnitMeshFem::CreateLines(9);
    ...
    gdm.mKappas.resize(cells.Size(), integration.GetNumIntegrationPoints());
    gdm.mKappas(cells.Size() / 2, 0) = 3 * ft / E; // imperfection
    ...
    constraints.Add(d, Constraint::Component(mesh.NodesAtAxis(eDirection::X, d, 1), {eDirection::X},
                                             Constraint::RhsRamp(1, 0.1)));

    QuasistaticSolver problem(equations, {d, eeq});
    problem.SetConstraints(constraints);
    for (double t = 0; t < 1; t += 0.01)
        problem.DoStep(t, "EigenSparseLU");
}
...
//
//    BOOST_CHECK_NO_THROW(TestStructure2D(NuTo::Interpolation::eShapeType::TRIANGLE2D, true,
