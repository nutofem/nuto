...
Teuchos::RCP<Tpetra::CrsMatrix<double, int, int>> ConversionTools::convertEigen2TpetraCrsMatrix(Eigen::SparseMatrix<double> rEigenMatrix, bool rAsGlobal, bool rFillComplete)
{
    ...
    {
        Teuchos::RCP<const Tpetra::Map<int, int>> rowMap = Teuchos::rcp(new Tpetra::Map<int, int>(Teuchos::OrdinalTraits<Tpetra::global_size_t>::invalid(), rowCount, 0, mComm_teuchos));
        Teuchos::RCP<const Tpetra::Map<int, int>> columnMap = Teuchos::rcp(new Tpetra::Map<int, int>(Teuchos::OrdinalTraits<Tpetra::global_size_t>::invalid(), columnCount, 0, mComm_teuchos));
        return convertEigen2TpetraCrsMatrix(rEigenMatrix, rowMap, columnMap, rFillComplete);
    }
}
...
void ConversionTools::generateDefaultGlobalMapsTpetra(Tpetra::Map<int, int> &rRowMap, Tpetra::Map<int, int> &rColumnMap, int rNumGlobalRows, int rNumGlobalColumns)
{
    ...
    Tpetra::Map<int, int> newColumnMap(rNumGlobalColumns, rNumGlobalColumns, 0, mComm_teuchos);
