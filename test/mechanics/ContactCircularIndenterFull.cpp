...
void ContactHertzCircularIndenterFull(const std::string &path,
                                      ...
                                      int &masterContactElementsGroupId)
{
    ...
    auto LambdaGetSlaveNodesLower = [](NuTo::NodeBase* rNodePtr) -> bool
    {
        ...
        if ( rNodePtr->GetNum(NuTo::Node::eDof::DISPLACEMENTS)>0 )
        {
            ...
            if (r <= radius + Tol && r >= radius - Tol && x >= -0.25 && x <= 0.25)//angle <= anglemax)
            {
                return true;
            }
        }
    ...
    auto LambdaGetNodesLeft = [](NuTo::NodeBase* rNodePtr) -> bool
    {
        ...
        if (rNodePtr->GetNum(NuTo::Node::eDof::COORDINATES)>0)
        {
            double x = rNodePtr->Get(NuTo::Node::eDof::COORDINATES)[0];
            double y = rNodePtr->Get(NuTo::Node::eDof::COORDINATES)[1];
            if ( y >= -0.5 - Tol && y <= -0.5 + Tol )//&& x >= -0.5-Tol && x <= -0.5+Tol )
            {
                return true;
            }
        }
...
//    countDBC++;
    ...
    auto LambdaGetMasterNodesUpper = [](NuTo::NodeBase* rNodePtr) -> bool
    {
        ...
        if (rNodePtr->GetNum(NuTo::Node::eDof::DISPLACEMENTS)>0)
        {
            double x = rNodePtr->Get(NuTo::Node::eDof::COORDINATES)[0];
            double y = rNodePtr->Get(NuTo::Node::eDof::COORDINATES)[1];
            if (y >= -1. - Tol && y <= -1. + Tol && x >= -0.25 - Tol && x <= 0.25 + Tol)
            {
                return true;
            }
        }
...
//    disp = 0.;
    ...
    for(int i = 0; i < rMembersSlaveDBC.rows(); i++)
    {
        ...
        myStructure->ConstraintLinearSetDisplacementNode(rMembersSlaveDBC(i), direction, 0.);
    }
         ...
         NuTo::eIntegrationType rIntegrationTypeViz)
{
    ...
    ContactHertzCircularIndenterFull(path,
                                     fileNameSlave,
                                     fileNameMaster,
                                     contactAlgo,
                                     myStructure,
...
int main(int argc, char* argv[])
{
    ...
    fileNameSlave  = "indenterCircular.msh";
    fileNameMaster = "Halfspace.msh";


    // ===> penalty parameter study <=== //
    resultDir = "./ResultsContactStaticCircularIndenter";
    penalty = 1.e10;

    contactAlgo = 1;
    resultDir = "./ResultsContactStaticCircularIndenter_CA1";
    penalty = 1.e10;
    run(resultDir,
        path,
        fileNameSlave,
