...
void SetDBC(NuTo::Structure &myStructure, int groupNodesSlave, int groupNodesMaster, int &countDBC)
{
    ...
    myStructure.GroupAddNodeFunction(groupNodesMasterUpper, LambdaNodes);
                 ...
                 int numElXMaster, int numElYMaster)
{
    ...
    buildStructure2D(rElementTypeIdent, rNumNodesPerElementInOneDir, nodeCoordinatesFirstElement, numElXMaster, numElYMaster, 1., 1., 0., -1., 0, &myStructure, groupNodesMaster, groupElementsMaster, setOfDOFSMaster);

    int countDBC;
    SetDBC(myStructure, -1, groupNodesMaster,  countDBC);
...
#ifdef ENABLE_VISUALIZE
    int visualizationGroup = groupElementsMaster;
    ...
    visualizationGroup = groupElementsIGAlayer;
    myStructure.AddVisualizationComponent(visualizationGroup, NuTo::eVisualizeWhat::DISPLACEMENTS);
    myStructure.ElementGroupExportVtkDataFile(visualizationGroup, resultDir+"/ElementsLayer.vtu", true);
#endif
}
...
int main()
{
    ...
    resultDir = "./ResultsStaticElementElementTest";
    degree = 2;
