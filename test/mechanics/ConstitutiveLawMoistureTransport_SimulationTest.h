                           ...
                           std::array<double,TDim> rL)
{
    ...
    BOOST_FOREACH(NodeMap::const_iterator::value_type it, nodePtrMap)
    {
        ...
        double relevantNodeCoord = nodePtr->GetCoordinates()[relevantDirection];
    ...
    {
        throw NuTo::Exception(__PRETTY_FUNCTION__,"One ore more calculated values exceeds the tolerance when compared to reference values");
    }
                    ...
                    bool rTestInterpolationTypeCombi = false)
{
    ...
    auto LambdaTimeDepBoundaryRH= [&MT,&timeControl]
                                  (double rTime)->double
                                {
                                    if(rTime == 0.0)
                                    {
