...
NuTo::Structure* buildStructure1D(double& DisplacementCorrect, int refinements)
{
    ...
    myStructure->ConstitutiveLawSetParameterDouble(Material, NuTo::Constitutive::eConstitutiveParameter::YOUNGS_MODULUS,
                                                   YoungsModulus);
    ...
    //    NuTo::Interpolation::eTypeOrder::SPLINE, vecDegree, vecKnots, curve.GetWeights());
    ...
    //        myStructure->ElementCreate(interpolationType, elementIncidence, curve.GetElementKnots(element),
    //        curve.GetElementKnotIDs(element));
...
NuTo::Structure* buildStructure1DConstraintsElements(double& DisplacementCorrect, int refinements)
{
    ...
    int countDBC = 0;
    myStructure->ConstraintLinearEquationCreate(countDBC, members(members.rows() - 1), NuTo::Node::eDof::DISPLACEMENTS,
                                                0, 1., 0.);
...
NuTo::Structure* buildStructure1DConstraintsWithoutElements(const std::string& resultDir, const std::string& name,
                                                            double& DisplacementCorrect)
{
    ...
    double nodeDisp =
            myStructure->NodeGetNodePtr(myStructure->GetNumNodes() - 1)->Get(NuTo::Node::eDof::DISPLACEMENTS)[0];
...
NuTo::Structure* constantStress(double& DisplacementCorrect, int refinements, const std::string& resultDir)
{
    ...
    myStructure->ConstitutiveLawSetParameterDouble(myMatLin, NuTo::Constitutive::eConstitutiveParameter::YOUNGS_MODULUS,
                                                   YoungsModulus);
    myStructure->ConstitutiveLawSetParameterDouble(myMatLin, NuTo::Constitutive::eConstitutiveParameter::POISSONS_RATIO,
                                                   PoissonRatio);
    ...
    surface.buildIGAStructure(*myStructure, setOfDOFS, groupElements, groupNodes, "IGA2D");
...
Eigen::VectorXd exact_plate_hole_disp(Eigen::VectorXd pt)
{
    ...
    disp(0) = ((T * a) / (8. * mu)) * ((r / a) * (k + 1.) * ct + ((2. * a) / r) * ((1. + k) * ct + c3t) -
                                       ((2. * a * a * a) / (r * r * r)) * c3t);
    disp(1) = ((T * a) / (8. * mu)) * ((r / a) * (k - 3.) * st + ((2. * a) / r) * ((1. - k) * st + s3t) -
                                       ((2. * a * a * a) / (r * r * r)) * s3t);

    //    disp(0) = ((8.*a*(1 + nu)*T)/E) * ((r/a)*(2./(1+nu))*ct + (a/r)*((4./(1+nu))*ct + c3t) -
    //    (a/r)*(a/r)*(a/r)*c3t);
    //    disp(1) = ((8.*a*(1 + nu)*T)/E) * ((r/a)*((-2.*nu)/(1+nu))*st + (a/r)*(((-4.*nu)/(1+nu))*st + s3t) -
    //    (a/r)*(a/r)*(a/r)*s3t);
...
NuTo::Structure* buildPlateWithHole2DNeumann(const std::string& resultDir, const std::string& name, int refine, int BC,
                                             bool constraint)
{
    NuTo::Structure* myStructure = new NuTo::Structure(2);
    ...
    if (constraint)
    {
        ...
        {
            myStructure->ConstraintLinearEquationCreate(countDBC, membersCorner(0), NuTo::Node::eDof::DISPLACEMENTS,
                                                        dof, 1., 0.);
            myStructure->ConstraintLinearEquationAddTerm(countDBC, membersCorner(1), NuTo::Node::eDof::DISPLACEMENTS,
                                                         dof, -1.);
            countDBC++;
        }
    }
    ...
    std::cout << constraint << "====================================\n";
    if (membersCorner.rows() > 2)
        throw NuTo::Exception("[IGA] : there have to be 2 corner nodes!");
...
void solve(NuTo::Structure* myStructure, double solution, const std::string& resultDir, const std::string& name,
           bool exc, double tol = 1.e-6)
{
    myStructure->SolveGlobalSystemStaticElastic();
...
void Neumann(const std::string& resultDir, const std::string& path, const std::string& fileName, int BC)
{
    ...
    NuTo::FullMatrix<int, Eigen::Dynamic, Eigen::Dynamic> groupIndices =
            myStructure.ImportFromGmsh(path + fileName, NuTo::ElementData::eElementDataType::CONSTITUTIVELAWIP,
    ...
    //    NuTo::IntegrationType::IntegrationType2D3NGauss3Ip, NuTo::IpData::NOIPDATA);
    ...
    myStructure.ConstitutiveLawSetParameterDouble(constitutiveLaw,
                                                  NuTo::Constitutive::eConstitutiveParameter::YOUNGS_MODULUS, 1.e5);
    myStructure.ConstitutiveLawSetParameterDouble(constitutiveLaw,
                                                  NuTo::Constitutive::eConstitutiveParameter::POISSONS_RATIO, 0.3);
...
int main()
{
    ...
    myStructure = buildPlateWithHole2DNeumann(resultDir, "Hole1", 1, 0, false);
