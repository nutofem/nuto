...
fakeit::Mock<NuTo::CellInterface> MockCell(const NuTo::DofType& dof, Eigen::Vector3i numbering)
{
    ...
    Method(cell, Gradient) = mockGradient;
    Method(cell, Hessian0) = mockHessian;
...
BOOST_AUTO_TEST_CASE(AssemberGradient)
{
    ...
    NuTo::GlobalDofVector gradient = assembler.BuildGradient({&mockCell0.get(), &mockCell1.get()}, {&d});
...
BOOST_AUTO_TEST_CASE(AssemberHessian)
{
    ...
    NuTo::GlobalDofMatrixSparse hessian = assembler.BuildHessian0({&mockCell0.get(), &mockCell1.get()}, {&d});
