...
{
    for (boost::ptr_list<Transformation>::const_reverse_iterator it = mlTransformationInput.rbegin();
         it != mlTransformationInput.rend(); it++)
...
{
    for (boost::ptr_list<Transformation>::const_reverse_iterator it = mlTransformationOutput.rbegin();
         it != mlTransformationOutput.rend(); it++)
        it->TransformBackward(rCoordinates);
}
...
void NuTo::SupportPoints::GetPearsonCorrelationMatrixTransformed(Eigen::MatrixXd& rCorrelationMatrix) const
{
    if (!this->mTransformationBuild)
    {
        throw NuTo::Exception("[NuTo::SupportPoints::GetPearsonCorrelationMatrixTransformed] transformation "
...
void NuTo::SupportPoints::GetPearsonCorrelationMatrixConfidenceIntervalsOriginal(Eigen::MatrixXd& rCorrelationMatrix,
...
void NuTo::SupportPoints::GetPearsonCorrelationMatrixConfidenceIntervalsTransformed(
        ...
        Eigen::MatrixXd& rMaxCorrelationMatrix, double rAlpha) const
{
    if (!this->mTransformationBuild)
    {
        throw NuTo::Exception("[NuTo::SupportPoints::"
 ...
 * \f]
 * where \f$p\f$ is the number of point coordinates, \f$n\f$ is the number of samples, and \f$\bar{x}_i\f$ is the sample
 * mean of the
 ...
 *    cov(x_1,x_1) & cov(x_1, x_2) & \cdots & cov(x_1, x_{p_i}) & cov(x_1, y_1) & cov(x_1, y_2) & \cdots & cov(x_1,
 * y_{p_o})\\
 *    cov(x_2,x_1) & cov(x_2, x_2) & \cdots & cov(x_2, x_{p_i}) & cov(x_2, y_1) & cov(x_2, y_2) & \cdots & cov(x_2,
 * y_{p_o})\\
 *    \vdots & \vdots & \ddots & \vdots & \vdots & \vdots & \ddots & \vdots\\
 *    cov(x_{p_i},x_1) & cov(x_{p_i}, x_2) & \cdots & cov(x_{p_i}, x_{p_i}) & cov(x_{p_i}, y_1) & cov(x_{p_i}, y_2) &
 * \cdots & cov(x_{p_i}, y_{p_o})\\
 *    cov(y_1,x_1) & cov(y_1, x_2) & \cdots & cov(y_1, x_{p_i}) & cov(y_1, y_1) & cov(y_1, y_2) & \cdots & cov(y_1,
 * y_{p_o})\\
 *    cov(y_2,x_1) & cov(y_2, x_2) & \cdots & cov(y_2, x_{p_i}) & cov(y_2, y_1) & cov(y_2, y_2) & \cdots & cov(y_2,
 * y_{p_o})\\
 *    \vdots & \vdots & \ddots & \vdots & \vdots & \vdots & \ddots & \vdots\\
 *    cov(y_{p_o},x_1) & cov(y_{p_o}, x_2) & \cdots & cov(y_{p_o}, x_{p_i}) & cov(y_{p_o}, y_1) & cov(y_{p_o}, y_2) &
 * \cdots & cov(y_{p_o}, y_{p_o})
 *  \end{bmatrix},
 * \f]
 * with
 * \f[
 ...
 * \f]
 * where \f$\bar{z}_i\f$ is the sample mean of the \f$i\f$-th coordinate, \f$n\f$ is the number of samples, \f$p_i\f$ is
                                                    ...
                                                    Eigen::MatrixXd& rCovarianceMatrix) const
{
    ...
    {
        throw Exception("[NuTo::SupportPoints::CalculateCovarianceMatrix] number of samples in input data and "
 ...
 *    \rho_{x_1,x_1} & \rho_{x_1, x_2} & \cdots & \rho_{x_1, x_{p_i}} & \rho_{x_1, y_1} & \rho_{x_1, y_2} & \cdots &
 * \rho_{x_1, y_{p_o}}\\
 *    \rho_{x_2,x_1} & \rho_{x_2, x_2} & \cdots & \rho_{x_2, x_{p_i}} & \rho_{x_2, y_1} & \rho_{x_2, y_2} & \cdots &
 * \rho_{x_2, y_{p_o}}\\
 *    \vdots & \vdots & \ddots & \vdots & \vdots & \vdots & \ddots & \vdots\\
 *    \rho_{x_{p_i},x_1} & \rho_{x_{p_i}, x_2} & \cdots & \rho_{x_{p_i}, x_{p_i}} & \rho_{x_{p_i}, y_1} & \rho_{x_{p_i},
 * y_2} & \cdots & \rho_{x_{p_i}, y_{p_o}}\\
 *    \rho_{y_1,x_1} & \rho_{y_1, x_2} & \cdots & \rho_{y_1, x_{p_i}} & \rho_{y_1, y_1} & \rho_{y_1, y_2} & \cdots &
 * \rho_{y_1, y_{p_o}}\\
 *    \rho_{y_2,x_1} & \rho_{y_2, x_2} & \cdots & \rho_{y_2, x_{p_i}} & \rho_{y_2, y_1} & \rho_{y_2, y_2} & \cdots &
 * \rho_{y_2, y_{p_o}}\\
 *    \vdots & \vdots & \ddots & \vdots & \vdots & \vdots & \ddots & \vdots\\
 *    \rho_{y_{p_o},x_1} & \rho_{y_{p_o}, x_2} & \cdots & \rho_{y_{p_o}, x_{p_i}} & \rho_{y_{p_o}, y_1} & \rho_{y_{p_o},
 * y_2} & \cdots & \rho_{y_{p_o}, y_{p_o}}
 *  \end{bmatrix}
 * \f]
 * with the sample correlation coefficient \f$\rho_{z_i,z_j}\f$ given by
 * \f[
 * \rho_{z_i,z_j} = \dfrac{cov(z_i,z_j)}{\sigma_{z_i} \sigma_{z_j}} = \dfrac{1}{n-1} \sum_{k=1}^n \left[\dfrac{z_{ik} -
 * \bar{z}_i}{\sigma_{z_i}} \right]\left[\dfrac{z_{jk} - \bar{z}_j}{\sigma_{z_j}} \right],
 * \f]
 * where \f$cov(z_i,z_j)\f$ is the covariance of the variables \f$z_i\f$ and \f$z_j\f$, \f$\sigma_{z_i}, \sigma_{z_j}\f$
 * are the corresponding sample standard deviations,
 * \f$\bar{z}_i, \bar{z}_j\f$ are the corresponding sample means, \f$n\f$ is the number of samples, \f$p_i\f$ is the
 * number of input coordinates,
 * and \f$p_o\f$ is the number of output coordinates.
 */
void NuTo::SupportPoints::CalculatePearsonCorrelationMatrix(const Eigen::MatrixXd& rInputData,
                                                            const Eigen::MatrixXd& rOutputData,
 ...
 * \f[
 *  \tanh \left( \text{arctanh} (\rho) - \dfrac{Z_{0.5\alpha}}{\sqrt{n-3}} \right) \leq \rho \leq \tanh \left(
 * \text{arctanh} (\rho) + \dfrac{Z_{0.5\alpha}}{\sqrt{n-3}} \right),
 * \f]
        ...
        Eigen::MatrixXd& rMinCorrelationMatrix, Eigen::MatrixXd& rMaxCorrelationMatrix, double rAlpha) const
{
    ...
    {
        throw NuTo::Exception("[NuTo::SupportPoints::CalculatePearsonCorrelationMatrixConfidenceIntervals] at "
