...
void NuTo::SparseDirectSolverPardiso::Solve(const NuTo::SparseMatrixCSR<double>& rMatrix, const NuTo::FullVector<double, Eigen::Dynamic> &rRhs, NuTo::FullVector<double, Eigen::Dynamic> &rSolution)
{
    ...
    if (error != 0)
    {
        throw NuTo::MathException(__PRETTY_FUNCTION__, "Numerical factorization phase: " + this->GetErrorString(error) + ".");
    }
    ...
    if (error != 0)
    {
        throw NuTo::MathException(__PRETTY_FUNCTION__, "Back substitution and iterative refinement phase: " + this->GetErrorString(error) + ".");
    }
