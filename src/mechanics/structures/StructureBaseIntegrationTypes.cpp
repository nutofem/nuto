...
#include "mechanics/integrationtypes/IntegrationType1D2NConstVariableIp.h"
...
#include "mechanics/integrationtypes/IntegrationType2D4NModTriangle.h"
...
with variable number of Ips is e.g. 2D4NMOD100IP12.");
            ...
            it = mIntegrationTypeMap.insert(const_cast<std::string&>(IntegrationTypeString), new NuTo::IntegrationType2D4NModVariableIp(const_cast<std::string&>(IntegrationTypeString), numIp)).first;
            return it->second;
        } else if (IntegrationTypeString.substr(0, 7) == "2D4NTRI")
        {
            ...
            it = mIntegrationTypeMap.insert(const_cast<std::string&>(IntegrationTypeString), new NuTo::IntegrationType2D4NModTriangle(const_cast<std::string&>(IntegrationTypeString))).first;
            return it->second;
        } else
        {
            throw MechanicsException("[NuTo::StructureBase::GetPtrIntegrationType] Integration type " + IntegrationTypeString + " does not exist. Create the integration type before it is used.");
