...
NuTo::MisesPlasticityEngineeringStress::MisesPlasticityEngineeringStress()
    : ConstitutiveBase()
{
    mE = 0.;
    mNu = 0.;
...
template <>
void NuTo::MisesPlasticityEngineeringStress::Evaluate<2>(const ConstitutiveInputMap& rConstitutiveInput,
                                                         const ConstitutiveOutputMap& rConstitutiveOutput,
                                                         Data& rStaticData)
{
...
template <>
void NuTo::MisesPlasticityEngineeringStress::Evaluate<3>(const ConstitutiveInputMap& rConstitutiveInput,
                                                         const ConstitutiveOutputMap& rConstitutiveOutput,
                                                         Data& rStaticData)
{
...
bool NuTo::MisesPlasticityEngineeringStress::CheckDofCombinationComputable(Node::eDof rDofRow, Node::eDof rDofCol,
                                                                           int rTimeDerivative) const
{
    assert(rTimeDerivative > -1);
    if (rTimeDerivative < 1 && rDofRow == Node::eDof::DISPLACEMENTS && rDofCol == Node::eDof::DISPLACEMENTS)
    {
        return true;
    }
    return false;
...
void NuTo::MisesPlasticityEngineeringStress::ReturnMapping2D(StaticData::DataMisesPlasticity<3>& oldStaticData,
...
void NuTo::MisesPlasticityEngineeringStress::ReturnMapping3D(StaticData::DataMisesPlasticity<3>& oldStaticData,
                                                             ...
                                                             StaticData::DataMisesPlasticity<3>* rNewStaticData) const
{
    ...
    double sigma_trial[6], xi_trial[6], norm_dev, sigma_y, factor, yield_condition, d_sigma, d_H, H, H2, mu,
            bulk_modulus, delta_gamma = 0., df_dsigma[6], trace_epsilon, trace_epsilon_div_3;
    ...
    if (i == 100)
    {
        ...
        std::cout << "total strain " << total_strain[0] << " " << total_strain[1] << " " << total_strain[2] << " "
                  << total_strain[3] << " " << total_strain[4] << " " << total_strain[5] << " "
                  << "\n";
        std::cout << "plastic strain " << plastic_strain[0] << " " << plastic_strain[1] << " " << plastic_strain[2]
                  << " " << plastic_strain[3] << " " << plastic_strain[4] << " " << plastic_strain[5] << " "
                  << "\n";
        std::cout << "back stress" << back_stress[0] << " " << back_stress[1] << " " << back_stress[2] << " "
                  << back_stress[3] << " " << back_stress[4] << " " << back_stress[5] << " "
                  << "\n";
        std::cout << "[NuTo::MisesPlasticityEngineeringStress::ReturnMapping3D] No convergence after 100 steps, check "
                     "the source code."
                  << "\n";
...
void NuTo::MisesPlasticityEngineeringStress::CheckYieldStrength(std::vector<std::pair<double, double>> rSigma) const
{
	...
	for (unsigned int count=1; count<rSigma.size(); count++)
	{
		if (rSigma[count-1].first>=rSigma[count].first)
	        throw NuTo::Exception(__PRETTY_FUNCTION__, "For multilinear plasticity, the epsilon should always increase.");
		if (rSigma[count-1].second>rSigma[count].second)
	        throw NuTo::Exception(__PRETTY_FUNCTION__, "For multilinear plasticity, the yield strength should always increase.");
	}
}
...
void NuTo::MisesPlasticityEngineeringStress::CheckHardeningModulus(std::vector<std::pair<double, double>> rH) const
{
	...
	for (unsigned int count=1; count<rH.size(); count++)
	{
		...
		if (rH[count].second<0)
	        throw NuTo::Exception(__PRETTY_FUNCTION__, "For multilinear plasticity, the hardening modulus should always be positive.");
	}
}
