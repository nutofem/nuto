...
void NuTo::LinearElasticInhomogeneous::UpdateParameters(
    Eigen::VectorXd coordinates) {
  SetParameterDouble(Constitutive::eConstitutiveParameter::YOUNGS_MODULUS,
                     mEfunc(coordinates));
}

void NuTo::LinearElasticInhomogeneous::SetYoungsModulus(
    std::function<double(Eigen::VectorXd)> E) {
  mEfunc = E;
}

NuTo::ConstitutiveInputMap
NuTo::LinearElasticInhomogeneous::GetConstitutiveInputs(
...
void LinearElasticInhomogeneous::Evaluate(
  ...
  Eigen::MatrixXd coordinates =
      rConstitutiveInput.at(Constitutive::eInput::COORDINATES)
          ->CopyToEigenMatrix();
  UpdateParameters(coordinates);

  LinearElasticEngineeringStress::Evaluate<TDim>(rConstitutiveInput,
                                                 rConstitutiveOutput);
}
