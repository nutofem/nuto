...
{
    SetParameterDouble(Constitutive::eConstitutiveParameter::YOUNGS_MODULUS, mEfunc(coordinates));
}
...
NuTo::LinearElasticInhomogeneous::GetConstitutiveInputs(const ConstitutiveOutputMap& rConstitutiveOutput) const
{
...
template <int TDim>
void LinearElasticInhomogeneous::Evaluate(const ConstitutiveInputMap& rConstitutiveInput,
                                          const ConstitutiveOutputMap& rConstitutiveOutput)
{
    Eigen::MatrixXd coordinates = rConstitutiveInput.at(Constitutive::eInput::COORDINATES)->CopyToEigenMatrix();
    UpdateParameters(coordinates);

    LinearElasticEngineeringStress::Evaluate<TDim>(rConstitutiveInput, rConstitutiveOutput);
}

template void LinearElasticInhomogeneous::Evaluate<1>(const ConstitutiveInputMap& rConstitutiveInput,
                                                      const ConstitutiveOutputMap& rConstitutiveOutput);
template void LinearElasticInhomogeneous::Evaluate<2>(const ConstitutiveInputMap& rConstitutiveInput,
                                                      const ConstitutiveOutputMap& rConstitutiveOutput);
template void LinearElasticInhomogeneous::Evaluate<3>(const ConstitutiveInputMap& rConstitutiveInput,
                                                      const ConstitutiveOutputMap& rConstitutiveOutput);
