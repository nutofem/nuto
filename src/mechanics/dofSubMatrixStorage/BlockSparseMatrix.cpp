...
NuTo::BlockSparseMatrix::BlockSparseMatrix(const DofStatus& rDofStatus, bool rCanBeSymmetric)
    ...
    , mCanBeSymmetric(rCanBeSymmetric)
{
    AllocateSubmatrices();
}
    ...
    , mCanBeSymmetric(rOther.mCanBeSymmetric)
{
    for (const auto& it : rOther.mData)
    {
        if (it.second->IsSymmetric())
            mData[it.first] = std::unique_ptr<SparseMatrixCSRVector2<double>>(
                    new SparseMatrixCSRVector2Symmetric<double>(it.second->AsSparseMatrixCSRVector2Symmetric()));
        else
            mData[it.first] = std::unique_ptr<SparseMatrixCSRVector2<double>>(
                    new SparseMatrixCSRVector2General<double>(it.second->AsSparseMatrixCSRVector2General()));
    }
}
    ...
    for (auto dofRow : dofTypes)
        for (auto dofCol : dofTypes)
        {
            if (mCanBeSymmetric and dofRow == dofCol and mDofStatus.IsSymmetric(dofRow))
                mData[std::make_pair(dofRow, dofCol)] = std::unique_ptr<SparseMatrixCSRVector2<double>>(
                        new SparseMatrixCSRVector2Symmetric<double>(0, 0));
            else
                mData[std::make_pair(dofRow, dofCol)] = std::unique_ptr<SparseMatrixCSRVector2<double>>(
                        new SparseMatrixCSRVector2General<double>(0, 0));
...
const NuTo::SparseMatrixCSRVector2<double>& NuTo::BlockSparseMatrix::operator()(Node::eDof rDofRow,
...
void NuTo::BlockSparseMatrix::Resize(const std::map<Node::eDof, int>& rNumRowDofsMap,
                                     const std::map<Node::eDof, int>& rNumColumnDofsMap)
{
    const auto& dofTypes = mDofStatus.GetDofTypes();
    for (auto dofRow : dofTypes)
        for (auto dofCol : dofTypes)
...
#pragma omp parallel for schedule(static)
                ...
                for (int iRow = 0; iRow < numRowsDof; ++iRow)
                {
                    ...
                    for (unsigned int iCol = 0; iCol < dofVals[iRow].size(); ++iCol)
                    {
                        ...
                        cols[blockStartRow + iRow].push_back(dofCols[iRow][iCol] +
                                                             blockStartCol); // corrector for the column numbering
                    }
                }
            }
            blockStartCol += subMatrix.GetNumColumns();
...
std::unique_ptr<NuTo::SparseMatrixCSR<double>> NuTo::BlockSparseMatrix::ExportToCSR() const
{
    ...
    if (activeDofs.size() == 1 && mDofStatus.IsSymmetric(dof))
    {
        ...
        std::unique_ptr<NuTo::SparseMatrixCSR<double>> matrix =
                std::make_unique<SparseMatrixCSRSymmetric<double>>(ref.AsSparseMatrixCSRVector2Symmetric());
