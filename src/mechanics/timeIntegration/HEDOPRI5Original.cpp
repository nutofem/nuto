...
NuTo::HEDOPRI5Original::HEDOPRI5Original(StructureBase* rStructure)
...
NuTo::Error::eError NuTo::HEDOPRI5Original::Solve(double rTimeDelta)
{
    ...
    try
    {
        ...
        Eigen::VectorXd disp_j, vel_j, tmp_k, disp_j_tmp,
                vel_j_tmp, // intermediate values of the displacements and velocities
                disp_j_new, vel_j_new; // new d and v at end of time step
        std::vector<Eigen::VectorXd> d_disp_j_tmp, d_vel_j_tmp; // intermediate values of the time derivatives of d and
                                                                // v
        ...
        std::cout << "the total mass is "
                  << intForce_j.sum() / ((double)mStructure->GetDimension()) +
                             tmp_k.sum() / ((double)mStructure->GetDimension())
                  << std::endl;
        ...
        while (curTime < rTimeDelta)
        {
            ...
            std::cout << "curTime " << curTime << " (" << curTime / rTimeDelta << ") max Disp = " << disp_j.maxCoeff()
                      << std::endl;
            disp_j_new = disp_j;
            vel_j_new = vel_j;
                ...
                if (contactAreas > 0 && countStage > 0)
                {
                    for (int contact = 0; contact < contactAreas; contact++)
                    {
                        if (!mStructure->IsPenalty(contact))
                        {
                            ...
                            mStructure->SetConstraintContactVector(mConstraintContact[contact],
                                                                   contactContributionVectorDisp, 0);
                            mStructure->SetConstraintContactVector(mConstraintContact[contact],
                                                                   contactContributionVectorVel, 1);
                            ...
                            // final stage
                            disp_j_new -= tempDisp * (GetStageWeights(countStage - 1)) /
                                          stageDerivativeFactor[countStage - 1];
                            vel_j_new -=
                                    tempVel * (GetStageWeights(countStage - 1)) / stageDerivativeFactor[countStage - 1];
                ...
                if (contactAreas > 0)
                {
                    for (int contact = 0; contact < contactAreas; contact++)
                    {
                        if (mStructure->IsPenalty(contact))
                        {
                            ...
                            d_vel_j_tmp[countStage] +=
                                    mTimeStep * (invMassMatrix_j.asDiagonal()) * contactContributionVectorVel;
                        }
                    }
                }
