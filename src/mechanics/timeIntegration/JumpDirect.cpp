...
NuTo::JumpDirect::JumpDirect(StructureBase* rStructure)
...
NuTo::Error::eError NuTo::JumpDirect::Solve(double rTimeDelta)
{
    ...
    try
    {
        ...
        /*
        // ConstraintGetConstraintMatrixAfterGaussElimination(const NuTo::SparseMatrixCSRGeneral<double>& ... ) replaced
        by
        ...
        */
        NuTo::SparseMatrixCSRVector2General<double> Cmat(
                mStructure->ConstraintGetConstraintMatrixAfterGaussElimination());
        SparseMatrixCSRVector2General<double> CmatT(Cmat.Transpose());
        Eigen::VectorXd bRHSprev, bRHSend;
        ...
        massMatrix_kk.Resize(mStructure->GetNumDofs() - mStructure->GetNumActiveDofs(),
                             mStructure->GetNumDofs() - mStructure->GetNumActiveDofs());
        mStructure->BuildGlobalCoefficientSubMatricesGeneral(NuTo::StructureEnum::eMatrixType::MASS, massMatrix_jj,
                                                             massMatrix_jk, massMatrix_kj, massMatrix_kk);

        Eigen::VectorXd extForce_j, extForce_k, prevExtForce_j, prevExtForce_k;
        Eigen::VectorXd cyclicPreJumpIntForce_j(mStructure->GetNumActiveDofs()),
                cyclicPreJumpIntForceNext_j(mStructure->GetNumActiveDofs()),
                cyclicPreJumpIntForce_k(mStructure->GetNumDofs() - mStructure->GetNumActiveDofs()),
                cyclicPreJumpIntForceNext_k(mStructure->GetNumDofs() - mStructure->GetNumActiveDofs());
        Eigen::VectorXd cyclicAfterJumpIntForce_j(mStructure->GetNumActiveDofs()),
                cyclicAfterJumpIntForce_k(mStructure->GetNumDofs() - mStructure->GetNumActiveDofs());
        Eigen::VectorXd cyclicPreJumpIntForce0_j(mStructure->GetNumActiveDofs()),
                cyclicPreJumpIntForce0_k(mStructure->GetNumDofs() - mStructure->GetNumActiveDofs());
        Eigen::VectorXd cyclicPreJumpIntForceMax_j(mStructure->GetNumActiveDofs()),
                cyclicPreJumpIntForceMax_k(mStructure->GetNumDofs() - mStructure->GetNumActiveDofs());
        Eigen::VectorXd residual_j, residual_k, residual_mod, prevResidual_j, prevResidual_k;
        Eigen::VectorXd vel_j, vel_k, acc_j, acc_k;
        // preJump values are stored for situation if the jump has to be repeated
        Eigen::VectorXd preJump_disp_Mean_j, preJump_disp_Mean_k, preJump_disp_Ampl_j, preJump_disp_Ampl_k,
                preJump_vel_j, preJump_vel_k, preJump_acc_j, preJump_acc_k;
        ...
        //    	//Example get damage
        //    	if (mStructure->ElementGetElementPtr(0)->GetConstitutiveLaw(0)->GetType() ==
        //    NuTo::Constitutive::DAMAGE_VISCO_PLASTICITY_HARDENING_ENGINEERING_STRESS){
        //    		std::cout << "Constitutive law = " <<
        //    mStructure->ElementGetElementPtr(0)->GetConstitutiveLaw(0)->GetType() << std::endl;
        //    		std::cout << "Damage = " <<
        //    mStructure->ElementGetElementPtr(0)->GetStaticData(0)->AsDamageViscoPlasticity3D()->GetOmegaCompr() <<
        //    std::endl;
        //    	}

        // save static data, time and displacements after the 3th cycle for situation if the jump has to be repeated
        Eigen::VectorXd save_disp_j, save_disp_k, save_vel_j, save_vel_k, save_acc_j, save_acc_k;
        double SaveTime(mStructure->GetTime());
        mStructure->ElementFatigueSaveStaticData();
        //    	std::cout << "Saved statevs =======================" <<std::endl;		// delete me
        //    	if (mStructure->ElementGetElementPtr(189)->GetConstitutiveLaw(0)->GetType() ==
        //    NuTo::Constitutive::GRADIENT_DAMAGE_ENGINEERING_STRESS_FATIGUE){
        //    	 		std::cout << "Constitutive law = " <<
        //    mStructure->ElementGetElementPtr(189)->GetConstitutiveLaw(0)->GetType() << std::endl;
        //    	 		std::cout << "Damage  = " <<
        //    mStructure->ElementGetElementPtr(189)->GetStaticData(0)->AsGradientDamage2DFatigue()->GetOmega() <<
        //    std::endl;
        //    	 		std::cout << "DamageF = " <<
        //    mStructure->ElementGetElementPtr(189)->GetStaticData(0)->AsGradientDamage2DFatigue()->GetOmegaFatigue() <<
        //    std::endl;
        //    	 		std::cout << "Kappa   = " <<
        //    mStructure->ElementGetElementPtr(189)->GetStaticData(0)->AsGradientDamage2DFatigue()->GetKappa() <<
        //    std::endl;
        //    	 		std::cout << "KappaF  = " <<
        //    mStructure->ElementGetElementPtr(189)->GetStaticData(0)->AsGradientDamage2DFatigue()->GetKappaFatigue() <<
        //    std::endl;
        //    	 		std::cout << "NonLEq  = " <<
        //    mStructure->ElementGetElementPtr(189)->GetStaticData(0)->AsGradientDamage2DFatigue()->GetPrevNonlocalEqStrain()
        //    << std::endl;
        //    	 		std::cout << "NonLEqF = " <<
        //    mStructure->ElementGetElementPtr(189)->GetStaticData(0)->AsGradientDamage2DFatigue()->GetPrevNonlocalEqStrainFatigue()
        //    << std::endl;
        //    	 		std::cout << "PrevEp  = " <<
        //    mStructure->ElementGetElementPtr(189)->GetStaticData(0)->AsGradientDamage2DFatigue()->GetPrevStrain() <<
        //    std::endl;
        //    	 		std::cout << "PrevEpF = " <<
        //    mStructure->ElementGetElementPtr(189)->GetStaticData(0)->AsGradientDamage2DFatigue()->GetPrevStrainFatigue()
        //    << std::endl;
        //    	 		std::cout << "PrevS   = " <<
        //    mStructure->ElementGetElementPtr(189)->GetStaticData(0)->AsGradientDamage2DFatigue()->GetPrevStress() <<
        //    std::endl;
        //    	 		std::cout << "PrevSF  = " <<
        //    mStructure->ElementGetElementPtr(189)->GetStaticData(0)->AsGradientDamage2DFatigue()->GetPrevStressFatigue()
        //    << std::endl;
        //    	}
        ...
        //    	Eigen::VectorXd save_disp_j, save_disp_k, delta_disp_j, delta_disp_k;;
        //    	double SaveTime(mStructure->GetTime());
        //    	mStructure->NodeExtractDofValues(0,save_disp_j, save_disp_k);
        //    	mStructure->ElementFatigueSaveStaticData();
        //    	//calculate 10 cycles
        ...
        //    		std::cout << "Constitutive law = " <<
        //    mStructure->ElementGetElementPtr(0)->GetConstitutiveLaw(0)->GetType() << std::endl;
        //    		std::cout << "Damage after 10 cycles = " <<
        //    mStructure->ElementGetElementPtr(0)->GetStaticData(0)->AsDamageViscoPlasticity3D()->GetOmegaCompr() <<
        //    std::endl;
        //    	}
        ...
        //    	std::cout << " SaveTime " << SaveTime << ", mStructure->GetTime() = " << mStructure->GetTime() <<
        //    std::endl;
        //
        //    	//Example get damage
        //    	if (mStructure->ElementGetElementPtr(0)->GetConstitutiveLaw(0)->GetType() ==
        ...
        //    		std::cout << "Constitutive law = " <<
        //    mStructure->ElementGetElementPtr(0)->GetConstitutiveLaw(0)->GetType() << std::endl;
        //    		std::cout << "Damage after restoring " <<
        //    mStructure->ElementGetElementPtr(0)->GetStaticData(0)->AsDamageViscoPlasticity3D()->GetOmegaCompr() <<
        //    std::endl;
        //    	}
        //
        ...
        Eigen::VectorXd disp_Max_j, disp_Max_k, disp_Min_j, disp_Min_k;
        ...
        Eigen::VectorXd disp_Mean_j, disp_Mean_k, disp_Ampl_j, disp_Ampl_k;
        ...
        if (mStructure->GetNumTimeDerivatives() > 1)
        {
            mStructure->BuildGlobalGradientInternalPotentialSubVectors(cyclicPreJumpIntForce0_j,
                                                                       cyclicPreJumpIntForce0_k, false);

            mStructure->NodeExtractDofValues(1, preJump_vel_j, preJump_vel_k);
            mStructure->NodeExtractDofValues(2, preJump_acc_j, preJump_acc_k);
            //			std::cout << "=== acc_j = " << preJump_acc_j[0] << std::endl;
        }
        preJump_disp_Mean_j = disp_Mean_j;
        preJump_disp_Mean_k = disp_Mean_k;
        preJump_disp_Ampl_j = disp_Ampl_j;
        preJump_disp_Ampl_k = disp_Ampl_k;
        ...
        this->IntegrateSingleCycle(&disp_Mean_j, &disp_Mean_k, &disp_Ampl_j, &disp_Ampl_k,
                                   false); // the statevs and structure time evolve through the cycle
        ...
        //    << std::endl;
        //    	}
        mStructure->BuildGlobalGradientInternalPotentialSubVectors(
                cyclicPreJumpIntForce_j, cyclicPreJumpIntForce_k,
                false); // calculate cyclic change of the internal forces within the 5th cycle
        //    	std::cout << "Build Global Gradient Int Pot READY ==================" <<std::endl;		// delete
        //    me
        //    	if (mStructure->ElementGetElementPtr(189)->GetConstitutiveLaw(0)->GetType() ==
        //    NuTo::Constitutive::GRADIENT_DAMAGE_ENGINEERING_STRESS_FATIGUE){
        //    	 		std::cout << "Constitutive law = " <<
        ...
        //    	}
        mStructure->BuildGlobalGradientInternalPotentialSubVectors(
                cyclicPreJumpIntForceNext_j, cyclicPreJumpIntForceNext_k,
                false); // calculate cyclic change within the 6th cycle
        //    	std::cout << "Build Global Gradient Int Pot READY2 ==================" <<std::endl;		// delete
        //    me
        //    	if (mStructure->ElementGetElementPtr(189)->GetConstitutiveLaw(0)->GetType() ==
        //    NuTo::Constitutive::GRADIENT_DAMAGE_ENGINEERING_STRESS_FATIGUE){
        //    	 		std::cout << "Constitutive law = " <<
        ...
        if (mTimeDependentConstraint != -1)
        {
            timeDependentConstraintFactor =
                    this->CalculateTimeDependentConstraintFactor(SaveTime + 0.25 / mHarmonicFactor(0, 1));
        ...
        mStructure->BuildGlobalElasticGradientInternalPotentialSubVectors(cyclicPreJumpIntForceMax_j,
                                                                          cyclicPreJumpIntForceMax_k); // new place
        // the structure after the 3th cycle is already restored, except of displacements. Now restore the displacements
        // and
        ...
        // This is because the real cycle, especially at the beginning of the fatigue history, is not perfectly harmonic
        // (because statevs != const).
        // We are interesting in the change of the internal force during extrapolation, thus we have to subtract
        // the initial value of the internal force prior to extrapolation. In a static case this value is a zero and can
        // be ignored.
        mStructure->BuildGlobalElasticGradientInternalPotentialSubVectors(cyclicPreJumpIntForce0_j,
                                                                          cyclicPreJumpIntForce0_k); // new place
        ...
        //        NjumpDouble = mHarmonicExtrapolationTolerance*( cyclicPreJumpIntForceNext_j.array().abs() /
        //        (cyclicPreJumpIntForceNext_j - cyclicPreJumpIntForce_j).array().abs().max(mToleranceForce)
        //        ).minCoeff();
        NjumpDouble = mHarmonicExtrapolationTolerance *
                      (cyclicPreJumpIntForceNext_j.array().abs() /
                       (cyclicPreJumpIntForceNext_j - cyclicPreJumpIntForce_j).array().abs().maxCoeff())
                              .maxCoeff();
        Njump = std::max(NjumpMin, 1 + boost::numeric_cast<int>(NjumpDouble)); // convert to integer, but not less than
                                                                               // 3
        ...
        do
        {
            ...
            NjumpDouble = (rTimeDelta - SaveTime) *
                          mHarmonicFactor(0, 1); // number of cycles till the end of loading rTimeDelta
            ...
            if (NumberOfIterations >= 1 && Explicit == false)
            {
                ...
                // save the values of the previous iteration, required for the termination criterion of the implicit
                // iteration
            ...
            std::cout << "Time " << SaveTime << ", Iteration " << NumberOfIterations << ", Njump " << NjumpVector[0]
                      << std::endl;
            ...
            // find equilibrium: update mean and amplitude displacements																					//new
            // place
            this->CalculateFourierCoefficients(&disp_Mean_j, &disp_Mean_k, &disp_Ampl_j, &disp_Ampl_k,
                                               &cyclicPreJumpIntForce0_j, &cyclicPreJumpIntForce0_k,
                                               &cyclicPreJumpIntForceMax_j,
                                               &cyclicPreJumpIntForceMax_k); // the structure is set to
            // the updated mean displacements

            // find equilibrium: update mean and amplitude displacements																					//new
            // place
            this->CalculateFourierCoefficientsCoupledDofs(&disp_Mean_j, &disp_Mean_k, &disp_Ampl_j,
                                                          &disp_Ampl_k); // the structure is set to
            // the updated mean displacements
            ...
            //	    	std::cout << "After update static data =======================" <<std::endl;		// delete
            //me
            //	    	if (mStructure->ElementGetElementPtr(189)->GetConstitutiveLaw(0)->GetType() ==
            //NuTo::Constitutive::GRADIENT_DAMAGE_ENGINEERING_STRESS_FATIGUE){
            //	    	 		std::cout << "Constitutive law = " <<
            ...
            // no postprocessing, therefore=>false
            this->IntegrateSingleCycle(&disp_Mean_j, &disp_Mean_k, &disp_Ampl_j, &disp_Ampl_k,
                                       false); // the statevs evolve through the cycle
            mStructure->BuildGlobalGradientInternalPotentialSubVectors(cyclicAfterJumpIntForce_j,
                                                                       cyclicAfterJumpIntForce_k, false);
            ...
            JumpCriterionDouble = ((cyclicAfterJumpIntForce_j - cyclicPreJumpIntForce_j).array().abs() /
                                   cyclicAfterJumpIntForce_j.array().abs().maxCoeff())
                                          .maxCoeff();
            JumpCriterion = (JumpCriterionDouble <= mHarmonicExtrapolationTolerance);

            std::cout << "Maximal component of cyclic change of Fint = "
                      << ((cyclicAfterJumpIntForce_j - cyclicPreJumpIntForce_j).array().abs()).maxCoeff() << std::endl;

            //	        for (int i = 0; i <= sizeof(cyclicAfterJumpIntForce_j); ++i){
            //	        	std::cout << "i = " << i << " , cyclicPreJump = " << cyclicPreJumpIntForce_j[i] <<" ,
            //cycliAfterJump = " << cyclicAfterJumpIntForce_j[i] << std::endl;
            //	        }
            std::cout << "JumpCriterionDouble = " << JumpCriterionDouble << std::endl;

            if ((JumpCriterion && (Explicit || NumberOfIterations >= 1)) || Njump == NjumpMin ||
                ((Ralston || Midpoint) && NumberOfIterations == 1))
            {
                //	        if ((JumpCriterion && ( Explicit || NumberOfIterations >= 1)) || Njump == NjumpMin ) {
                ...
                //	        	//update velocities and accelerations due to slow evolution of the mean
                //displacement and amplitude
                //	        	//Explanation:
                //	        	//Let u(t) = (u0 + Du0 t/ N T) + (u1 + Du1 t / N T) sin wt
                //	        	//w = 2pi/T = 2pi*frequency, N = Njump, Du0, Du1 = changes of the mean
                //displacement and amplitude after the jump
                //	        	//The jump change of displacement due to the small changes Du0 and Du1 is equal
                //to:
                //	        	//Du = Du0 t / N T + (Du1 t / N T) sin wt
                //	        	//The jump change of velocity Ddu and acceleration Dddu due to the small changes
                //Du0 and Du1 are equal to:
                ...
                // update Njump
                NjumpDouble = Njump * std::sqrt(mHarmonicExtrapolationTolerance /
                                                std::max(JumpCriterionDouble, 0.25 * mHarmonicExtrapolationTolerance));
                if (Ralston)
                {
                    NjumpDouble *= 2. / 3.;
                    //	if (NjumpDouble < 3) NjumpDouble = 1.;			// to omit the loop, Njump = 3 -> Njump = 6 -> Njump
                    //= 3 ...
                }
                else if (Midpoint)
                {
                ...
                if (mTimeDependentConstraint != -1)
                {
                    timeDependentConstraintFactor = this->CalculateTimeDependentConstraintFactor(
                            SaveTime + 0.25 / mHarmonicFactor(0, 1)); // new place
                    mStructure->ConstraintSetRHS(mTimeDependentConstraint, timeDependentConstraintFactor); // new place
                } // new place
                mStructure->NodeMergeActiveDofValues(0, disp_Mean_j + disp_Ampl_j); // new place
                mStructure->ElementTotalUpdateTmpStaticData(); // new place
                mStructure->BuildGlobalElasticGradientInternalPotentialSubVectors(
                        cyclicPreJumpIntForceMax_j, cyclicPreJumpIntForceMax_k); // new place
                ...
                mStructure->BuildGlobalElasticGradientInternalPotentialSubVectors(
                        cyclicPreJumpIntForce0_j, cyclicPreJumpIntForce0_k); // new place

                // integrate single cycle for postprocessing=true and for the next extrapolation of statevs
                this->IntegrateSingleCycle(&disp_Mean_j, &disp_Mean_k, &disp_Ampl_j, &disp_Ampl_k,
                                           true); // the statevs, vel and acc evolve through the postprocessing
            ...
            else
            {
                ...
                disp_Mean_j = preJump_disp_Mean_j;
                disp_Mean_k = preJump_disp_Mean_k;
                disp_Ampl_j = preJump_disp_Ampl_j;
                disp_Ampl_k = preJump_disp_Ampl_k;
            ...
            //	        mStructure->ElementTotalUpdateTmpStaticData();
            //	    	std::cout << "mStress before USD" <<
            //mStructure->ElementGetElementPtr(0)->GetStaticData(0)->AsDamageViscoPlasticity3D()->GetPrevStress() <<
            //std::endl;
            //	    	std::cout << "mOmegaCompr before USD" <<
            //mStructure->ElementGetElementPtr(0)->GetStaticData(0)->AsDamageViscoPlasticity3D()->GetOmegaCompr() <<
            //std::endl;
            //			// the state variables are already extrapolated, except of mPrevStrain and mPrevSigma
            //			// call UpdateStaticData in order to update mPrevStrain and mPrevSigma too (this has to be done
            //after the equilibrium has been found)
            //	        mStructure->ElementTotalUpdateStaticData();
            //		    std::cout << "mStress after USD " <<
            //mStructure->ElementGetElementPtr(0)->GetStaticData(0)->AsDamageViscoPlasticity3D()->GetPrevStress() <<
            //std::endl;
            //		    std::cout << "mOmegaCompr after USD" <<
            //mStructure->ElementGetElementPtr(0)->GetStaticData(0)->AsDamageViscoPlasticity3D()->GetOmegaCompr() <<
            //std::endl;
...
//!@brief calculate the global stiffness matrix for the BVP with mean displacement (rFourier = 0) or with displacement
//!amplitude (rFourier = 1)
void NuTo::JumpDirect::CalculateGlobalModifiedStiffness(NuTo::SparseMatrixCSRVector2General<double>* rStiffnessMod,
                                                        int rFourierMode)
{
    switch (rFourierMode)
    {
    ...
    */
    ...
    SparseMatrixCSRVector2General<double> stiffMatrix_kk(mStructure->GetNumDofs() - mStructure->GetNumActiveDofs(),
                                                         mStructure->GetNumDofs() - mStructure->GetNumActiveDofs());
    mStructure->BuildGlobalElasticStiffnessSubMatricesGeneral(stiffMatrix_jj, stiffMatrix_jk, stiffMatrix_kj,
                                                              stiffMatrix_kk);
...
void NuTo::JumpDirect::CalculateFourierCoefficients(Eigen::VectorXd* rDisp_Mean_j, Eigen::VectorXd* rDisp_Mean_k,
                                                    Eigen::VectorXd* rDisp_Ampl_j, Eigen::VectorXd* rDisp_Ampl_k,
                                                    const Eigen::VectorXd* rIntForce_Mean_j,
                                                    const Eigen::VectorXd* rIntForce_Mean_k,
                                                    const Eigen::VectorXd* rIntForce_Max_j,
                                                    const Eigen::VectorXd* rIntForce_Max_k)
{
    Eigen::VectorXd extForce_j, extForce_k, extForceMax_j, extForceMax_k;
    Eigen::VectorXd intForce_j(mStructure->GetNumActiveDofs()),
            intForce_k(mStructure->GetNumDofs() - mStructure->GetNumActiveDofs()),
            intForceMax_j(mStructure->GetNumActiveDofs()),
            intForceMax_k(mStructure->GetNumDofs() - mStructure->GetNumActiveDofs());
    Eigen::VectorXd residual_j, residual_k, residual_mod;
    Eigen::VectorXd delta_disp_j, delta_disp_k, zero_displ_j(mStructure->GetNumActiveDofs());
    Eigen::VectorXd bRHS, bRHSmax;
    ...
    */
    ...
    {
        throw MechanicsException("[NuTo::JumpDirect::CalculateFourierCoefficients] the harmonic excitation can be only "
                                 "applied to the time dependent constraint or load case.");
    }

    // RemCom
    Eigen::VectorXd rIntForceX_Max_j, rIntForceX_Max_k, rIntForceX_Mean_j, rIntForceX_Mean_k, rDispX_Mean_j,
            rDispX_Mean_k, rDispX_Ampl_j, rDispX_Ampl_k;
    rIntForceX_Max_j = *rIntForce_Max_j;
    rIntForceX_Max_k = *rIntForce_Max_k;
    rIntForceX_Mean_j = *rIntForce_Mean_j;
    rIntForceX_Mean_k = *rIntForce_Mean_k;
    rDispX_Mean_j = *rDisp_Mean_j;
    rDispX_Mean_k = *rDisp_Mean_k;
    rDispX_Ampl_j = *rDisp_Ampl_j;
    rDispX_Ampl_k = *rDisp_Ampl_k;
    ...
    // calculate individual mass matrix
    NuTo::SparseMatrixCSRVector2General<double> massMatrix_jj, massMatrix_jk, massMatrix_kj, massMatrix_kk;
    Eigen::VectorXd lumped_massMatrix_j(mStructure->GetNumDofs()),
            lumped_massMatrix_k(mStructure->GetNumDofs() - mStructure->GetNumActiveDofs());
    // store last converged velocities and accelerations
    Eigen::VectorXd lastConverged_vel_j, lastConverged_vel_k, lastConverged_acc_j, lastConverged_acc_k;
    ...
    if (mStructure->GetNumTimeDerivatives() > 1 && FirstEqulibrium)
    {
        ...
        {
            massMatrix_jj.Resize(mStructure->GetNumActiveDofs(), mStructure->GetNumActiveDofs());
            massMatrix_jk.Resize(mStructure->GetNumActiveDofs(),
                                 mStructure->GetNumDofs() - mStructure->GetNumActiveDofs());
            massMatrix_kj.Resize(mStructure->GetNumDofs() - mStructure->GetNumActiveDofs(),
                                 mStructure->GetNumActiveDofs());
            massMatrix_kk.Resize(mStructure->GetNumDofs() - mStructure->GetNumActiveDofs(),
                                 mStructure->GetNumDofs() - mStructure->GetNumActiveDofs());
            mStructure->BuildGlobalCoefficientSubMatricesGeneral(NuTo::StructureEnum::eMatrixType::MASS, massMatrix_jj,
        ...
        if (mMuDampingMass > 0)
        {
            ...
            {
                residual_j +=
                        (massMatrix_jj * lastConverged_vel_j + massMatrix_jk * lastConverged_vel_k) * mMuDampingMass;
                residual_k +=
                        (massMatrix_kj * lastConverged_vel_j + massMatrix_kk * lastConverged_vel_k) * mMuDampingMass;
            }
        }
    ...
    *rDisp_Mean_k = bRHS - Cmat * (*rDisp_Mean_j);
    ...
    SparseMatrixCSRVector2General<double> K_jk(mStructure->GetNumActiveDofs(),
                                               mStructure->GetNumDofs() - mStructure->GetNumActiveDofs());
    SparseMatrixCSRVector2General<double> K_kj(mStructure->GetNumDofs() - mStructure->GetNumActiveDofs(),
                                               mStructure->GetNumActiveDofs());
    SparseMatrixCSRVector2General<double> K_kk(mStructure->GetNumDofs() - mStructure->GetNumActiveDofs(),
                                               mStructure->GetNumDofs() - mStructure->GetNumActiveDofs());
    ...
    if (DirectCalculation)
    {
        ...
        mStructure->ElementTotalUpdateTmpStaticData();
        mStructure->BuildGlobalElasticGradientInternalPotentialSubVectors(
                intForce_j, intForce_k); // calculate internal forces at zero displ.
    ...
    // we need to calculate the change of intForce(maximal displacement) - intForce(mean displacement) due to
    // extrapolation of statevs
    ...
    {
        extForce_j.Zero(mStructure->GetNumActiveDofs());
        extForce_k.Zero(mStructure->GetNumDofs() - mStructure->GetNumActiveDofs());
    }
    ...
    //	intForce_j.Zero(mStructure->GetNumActiveDofs()); intForce_k.Zero(mStructure->GetNumDofs() -
    //mStructure->GetNumActiveDofs());
    //	intForceMax_j.Zero(mStructure->GetNumActiveDofs()); intForceMax_k.Zero(mStructure->GetNumDofs() -
    //mStructure->GetNumActiveDofs());
    ...
    //	double
    //timeDependentConstraintFactorAmpl(this->CalculateTimeDependentConstraintFactor(MaximumHarmonicExcitation));
    ...
    // currently for the displacement controlled excitation, the external force is included into the mean equilibrium
    // only
    //	intForce_j.Zero(mStructure->GetNumActiveDofs()); intForce_k.Zero(mStructure->GetNumDofs() -
    //mStructure->GetNumActiveDofs());
    //	mStructure->BuildGlobalElasticGradientInternalPotentialSubVectors(intForce_j,intForce_k);
    ...
    residual_j.Zero(mStructure->GetNumActiveDofs());
    residual_k.Zero(mStructure->GetNumDofs() - mStructure->GetNumActiveDofs());
    ...
    //	if (mTimeDependentLoadCase != -1)									//new
    //place
    //	{
    //		residual_j -= extForce_j;
    //		residual_k -= extForce_k;
    ...
    //    NuTo::SparseMatrixCSRVector2General<double> massMatrix_jj,massMatrix_jk,massMatrix_kj,massMatrix_kk;
    //    Eigen::VectorXd lumped_massMatrix_j(mStructure->GetNumDofs()),
    //    		                                 lumped_massMatrix_k(mStructure->GetNumDofs() -
    //    mStructure->GetNumActiveDofs());
    ...
    *rDisp_Ampl_k = bRHS - Cmat * (*rDisp_Ampl_j);
    ...
    DirectCalculation = true; // displ controlled set true!, load controlled set false (or true, which overestimates
                              // than the displ. amplitude)
    if (DirectCalculation)
    {
    ...
    //
    //	std::cout << "mStructure->GetNumDofs() = " << mStructure->GetNumDofs() << " mStructure->GetNumActiveDofs() = "
    //<< mStructure->GetNumActiveDofs() << std::endl;
    //	std::cout << "sizeof(intForce_j) = " << sizeof(intForce_j) << " sizeof(intForce_k) = " << sizeof(intForce_k) <<
    //" sizeof(*rintForce_Max_j)" <<
    //			sizeof(*rIntForce_Max_j) << " sizeof(*rintForce_Max_k)" << sizeof(*rIntForce_Max_k) <<
    //std::endl;
    //	std::cout << "*rIntForce_Max_j -*rIntForce_Mean_j" << std::endl;
    //	std::cout << *rIntForce_Max_j - *rIntForce_Mean_j << std::endl;
    //	std::cout << " rDispX_Mean_j | rDispX_Ampl_j | delta_displ_j | rFintPrior_Max_j  |  FintAfterJump_Max_j  |
    //Delta_rFintPrior_j  |  Delta_FintAfterJump_j" << std::endl;
    //	for (int i = 0; i < mStructure->GetNumActiveDofs(); ++i) {
    //		std::cout << "i = " << i << " | " << rDispX_Mean_j[i] << " | " << rDispX_Ampl_j[i] << " | " <<
    //delta_disp_j[i]
    ...
    //	}
    //
    //	std::cout << " rDispX_Mean_k | rDispX_Ampl_k |rFintPrior_Max_k  |  FintAfterJump_Max_k  |  Delta_rFintPrior_k  |
    //Delta_FintAfterJump_k" << std::endl;
    //	for (int i = 0; i < mStructure->GetNumDofs() - mStructure->GetNumActiveDofs(); ++i) {
...
// NONLOCALEQSTRAIN
void NuTo::JumpDirect::CalculateFourierCoefficientsCoupledDofs(Eigen::VectorXd* rDisp_Mean_j,
                                                               Eigen::VectorXd* rDisp_Mean_k,
                                                               Eigen::VectorXd* rDisp_Ampl_j,
                                                               Eigen::VectorXd* rDisp_Ampl_k)
{
    ...
    // loop over those updatedDofs, which can be updated by this routine, in order to find out whether at least one Dof
    // from updatedDofs is active in the structure
    for (auto it = updatedDofs.begin(); it != updatedDofs.end(); it++)
    {
        if (mStructure->InterpolationTypeIsConstitutiveInput(it->first))
        {
            ...
            std::cout
                    << "[NuTo::JumpDirect::CalculateFourierCoefficientsCoupledDofs] the following Dof will be updated: "
                    << NuTo::Node::DofToString(it->first) << std::endl;
        }
    }
    ...
    //	for (auto it = updatedDofs.begin(); it != updatedDofs.end(); it++){
    //		if(it->second){
    //			std::cout<< "[NuTo::JumpDirect::CalculateFourierCoefficientsCoupledDofs] the following Dof will be updated"
    //<< NuTo::Node::AttributeToString(it->first) <<std::endl;
    //		}
    //	}
...
// Postprocessing should be done if the jump is acceptable; in this case the IntegrateSinleCycle should be repeated with
// a true option
void NuTo::JumpDirect::IntegrateSingleCycle(Eigen::VectorXd* rDisp_Mean_j, Eigen::VectorXd* rDisp_Mean_k,
                                            Eigen::VectorXd* rDisp_Ampl_j, Eigen::VectorXd* rDisp_Ampl_k,
                                            bool rIncludePostProcess)
{
    // calculate constraint matrix
    /*
    // ConstraintGetConstraintMatrixAfterGaussElimination(const NuTo::SparseMatrixCSRGeneral<double>& ... ) replaced by
    ...
    */
    NuTo::SparseMatrixCSRVector2General<double> Cmat(mStructure->ConstraintGetConstraintMatrixAfterGaussElimination());
    SparseMatrixCSRVector2General<double> CmatT(Cmat.Transpose());
    Eigen::VectorXd bRHSprev, bRHShalf, bRHSend, bRHSdot, bRHSddot;
    ...
    // calculate individual mass matrix (I was lazy here and have just used general matrices, but symmetric is certainly
    // better here)
    NuTo::SparseMatrixCSRVector2General<double> massMatrix_jj, massMatrix_jk, massMatrix_kj, massMatrix_kk;
    Eigen::VectorXd lumped_massMatrix_j(mStructure->GetNumDofs()),
            lumped_massMatrix_k(mStructure->GetNumDofs() - mStructure->GetNumActiveDofs());
    ...
    Eigen::VectorXd disp_j, disp_k, vel_j, vel_k, acc_j, acc_k;
    Eigen::VectorXd trial_disp_j, trial_disp_k, trial_vel_j, trial_vel_k, trial_acc_j, trial_acc_k;
    Eigen::VectorXd extForce_j, extForce_k, prevExtForce_j, prevExtForce_k;
    Eigen::VectorXd intForce_j(mStructure->GetNumActiveDofs()),
            intForce_k(mStructure->GetNumDofs() - mStructure->GetNumActiveDofs()),
    ...
    Eigen::VectorXd lastConverged_disp_j, lastConverged_disp_k, lastConverged_vel_j, lastConverged_vel_k,
            lastConverged_acc_j, lastConverged_acc_k;
    ...
    mStructure->NodeMergeActiveDofValues(
            0, lastConverged_disp_j); // disp_k is internally calculated from the previous constraint matrix
    mStructure->ElementTotalUpdateTmpStaticData();
...
//    }
    ...
    if (rIncludePostProcess)
    {
        ...
        //    	DamageFile << mTime << " " <<
        //    mStructure->ElementGetElementPtr(14)->GetStaticData(0)->AsDamageViscoPlasticity3D()->GetOmegaCompr() <<
        //    std::endl; // for Brick8N was ElementGetElementPtr(9) and ElementGetElementPtr(141) dlya Brick8Nhole;
        //    	TotalInelasticEqStrainFile << mTime << " " <<
        //    mStructure->ElementGetElementPtr(14)->GetStaticData(0)->AsDamageViscoPlasticity3D()->GetKappaInelastic()
        //    << std::endl; // for Brick8N was ElementGetElementPtr(9) and ElementGetElementPtr(141) dlya Brick8Nhole;
        // for 2d gradient model
        //    	DamageFile << mTime << " " <<
        //    mStructure->ElementGetElementPtr(189)->GetStaticData(0)->AsGradientDamage2DFatigue()->GetOmega()
        ...
        // 2D nonlocal test
        //        DamageFile << mTime << " " <<
        //        mStructure->ElementGetElementPtr(339)->GetStaticData(0)->AsGradientDamage2DFatigue()->GetOmega()
        //        		<< " " <<
        ...
        //				<<
        //mStructure->ElementGetElementPtr(406)->GetStaticData(0)->AsGradientDamage2DFatigue()->GetPrevNonlocalEqStrain()<<
        //std::endl;
    }
    ...
    for (int incr = 1; incr <= mHarmonicIncrementation; ++incr)
    {
        ...
        if (rIncludePostProcess)
        {
            ...
            if (mStructure->GetNumTimeDerivatives() > 1)
            {
                ...
                {
                    timeDependentConstraintFactor =
                            this->CalculateTimeDependentConstraintFactor(curTime - 0.5 * timeStep);
                    mStructure->ConstraintSetRHS(mTimeDependentConstraint, timeDependentConstraintFactor);
                }
                ...
                acc_j = (lastConverged_vel_j + lastConverged_acc_j * (timeStep * (0.5 - mBeta))) *
                        (-1. / (timeStep * mBeta));
                acc_j += (disp_j - lastConverged_disp_j) * (1. / (timeStep * timeStep * mBeta));
                vel_j = lastConverged_vel_j + lastConverged_acc_j * ((1. - mGamma) * timeStep) +
                        acc_j * (mGamma * timeStep);

                // calculate new accelerations and velocities of dependent dofs
                //                acc_k = bRHSddot - (Cmat*acc_j);
                //                vel_k = bRHSdot - (Cmat*vel_j);
                acc_k = (lastConverged_vel_k + lastConverged_acc_k * (timeStep * (0.5 - mBeta))) *
                        (-1. / (timeStep * mBeta));
                acc_k += bRHSdot * (1. / (timeStep * mBeta));
                vel_k = lastConverged_vel_k + lastConverged_acc_k * ((1. - mGamma) * timeStep) +
                        acc_k * (mGamma * timeStep);

                // direct calculation of vel and acc
                //        		vel_j = 2*pi*frequency*(*rDisp_Ampl_j)*cos(2*pi*frequency*(curTime -
                //        startingCycleTime));
                //        		acc_j = -pow(2*pi*frequency,2)*(*rDisp_Ampl_j)*sin(2*pi*frequency*(curTime -
                //        startingCycleTime));
                //
                //        		vel_k = 2*pi*frequency*(*rDisp_Ampl_k)*cos(2*pi*frequency*(curTime -
                //        startingCycleTime));
                //        		acc_k = -pow(2*pi*frequency,2)*(*rDisp_Ampl_k)*sin(2*pi*frequency*(curTime -
                //        startingCycleTime));
                ...
                else
                {
                    ...
                    //                  std::cout << "     RES.: prevIntForce_j + M*acc_j = " << prevResidual_j.Norm()
                    //                  << std::endl;
                }
            }
            ...
            //        	DamageFile << mTime << " " <<
            //        mStructure->ElementGetElementPtr(14)->GetStaticData(0)->AsDamageViscoPlasticity3D()->GetOmegaCompr()
            //        << std::endl; // for Brick8N was ElementGetElementPtr(9) and ElementGetElementPtr(141) dlya
            //        Brick8Nhole;
            //        	TotalInelasticEqStrainFile << mTime << " " <<
            //        mStructure->ElementGetElementPtr(14)->GetStaticData(0)->AsDamageViscoPlasticity3D()->GetKappaInelastic()
            //        << std::endl; // for Brick8N was ElementGetElementPtr(9) and ElementGetElementPtr(141) dlya
