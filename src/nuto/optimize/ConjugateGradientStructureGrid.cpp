 	...
 	if (matrixFreeMethod)
	{
		...
		for (int nodeNumber=0;nodeNumber<numNodes;++nodeNumber)
		{
			...
			else
			{
				for (int i=0;i<27;++i)
				{
					globNodeIds[27*nodeNumber+i]=-1;
					globArray[27*9*nodeNumber+i]= 0;
				}
...
/*
	    ...
	    */
		...
		if (mVerboseLevel>2)
		{
			...
			int width = 10;
			std::cout.precision(precision);
			std::cout <<std::setw(width)<< " norm gradient "<<normGradient<<std::endl;
			std::cout << std::setw(width)<< "gradient scaled " ;
			for (int count=0; count<GetNumParameters(); count++)
			{
				std::cout << std::setw(width)<< gradientScaled(count) << "   " ;
...
//			NodeBase* thisNode =mpGrid->NodeGetNodePtrFromGridNum(corners[node]);
			...
			NodeGrid3D* thisNode =mpGrid->NodeGridGetNodePtr(corners[node]);
			rHessian.SetValue(node*numDofs+0,node*numDofs+0,matrix->GetValue(node*numDofs+0,node*numDofs+0));
			rHessian.SetValue(node*numDofs+disp,node*numDofs+disp,matrix->GetValue(node*numDofs+disp,node*numDofs+disp));
			rHessian.SetValue(node*numDofs+disp,node*numDofs+disp,matrix->GetValue(node*numDofs+disp,node*numDofs+disp));
			}
        }
...
void NuTo::ConjugateGradientStructureGrid::CalculateStartGradient(NuTo::FullMatrix<double> &gradientOrig)
{
    ...
    {
    	//get pointer to each element
		thisElement=  mpGrid->ElementGetPtr(elementNumber);
		if (thisElement)
		{
			//get the number of the material
			int numStiff = thisElement->GetNumLocalStiffnessMatrix();
...
void NuTo::ConjugateGradientStructureGrid::CalculateStartGradientNodeByNode(NuTo::FullMatrix<double> &gradientOrig)
{
	...
	//		int numElems=thisNode->GetNumElems();
			...
			for (int element=0;element<8;++element)
			{
				...
				{
					thisElement=  mpGrid->ElementGetPtr(elementIds[element]);
			...
			*/
				if (nodeIds[node]>=0)
				{
					...
					if (mpGrid->NodeGetConstraintSwitch(nodeNumber*numDofs+0))
						gradientNew(nodeNumber*numDofs+0,0)+=array[node*9+0]*gradientOrig(nodeIds[node]*numDofs,0) + array[node*9+1]*gradientOrig(nodeIds[node]*numDofs+1,0) + array[node*9+2]*gradientOrig(nodeIds[node]*numDofs+2,0);
					if (mpGrid->NodeGetConstraintSwitch(nodeNumber*numDofs+1))
						gradientNew(nodeNumber*numDofs+1,0)+=array[node*9+3]*gradientOrig(nodeIds[node]*numDofs,0) + array[node*9+4]*gradientOrig(nodeIds[node]*numDofs+1,0) + array[node*9+5]*gradientOrig(nodeIds[node]*numDofs+2,0);
					if (mpGrid->NodeGetConstraintSwitch(nodeNumber*numDofs+2))
						gradientNew(nodeNumber*numDofs+2,0)+=array[node*9+6]*gradientOrig(nodeIds[node]*numDofs,0) + array[node*9+7]*gradientOrig(nodeIds[node]*numDofs+1,0) + array[node*9+8]*gradientOrig(nodeIds[node]*numDofs+2,0);
...
*/
	...
	//move all this in element loop
	Voxel8N* thisElement;
	thisElement=  mpGrid->ElementGetPtr(0);

	int dofsElem=thisElement->GetNumDofs();
	// return vector of all dofs
	FullMatrix<double> allReturn(GetNumParameters(),1);
	// return vector with all dofs of one element
	FullMatrix<double> locReturn(dofsElem,1);
...
*/
	...
	for (int nodeNumber=0;nodeNumber<numNodes;++nodeNumber)
    {
		...
		//get pointer to array of part coefficient matrix for all neighbor nodes
		array= &(globArray[nodeNumber*27*9]);
		//when node exists, at least one bool true
		if (constraint[nodeNumber*numDofs]||constraint[nodeNumber*numDofs+1]||constraint[nodeNumber*numDofs+2])
		{
			for (int node=0;node<27;++node)
			{
