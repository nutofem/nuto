...
//    start=clock();
//#endif
    	std::cout<<"[ConjugateGradientGridRed::HessianDiag] "<<std::endl;
	int numElems=mVoxelId.size();
	int nodeNumbers[3];
	...
	for (int elementNumber=0;elementNumber<numElems;elementNumber++)
    {
		...
		{
			rHessianDiag[3*mNodeId[mEdgesAtVoxel[8*mVoxelId[elementNumber]+node]]]+=(mYoungsModulus[mMaterialOfElem[elementNumber]]*mBaseStiffness[0]);
			rHessianDiag[3*mNodeId[mEdgesAtVoxel[8*mVoxelId[elementNumber]+node]]+1]+=(mYoungsModulus[mMaterialOfElem[elementNumber]]*mBaseStiffness[0]);
			rHessianDiag[3*mNodeId[mEdgesAtVoxel[8*mVoxelId[elementNumber]+node]]+2]+=(mYoungsModulus[mMaterialOfElem[elementNumber]]*mBaseStiffness[0]);
		}
    }
	for (int i=0;i<mNumParameters;++i)
	{
...
//#endif //SHOW_TIME
	...
	for (int elementNumber=0;elementNumber<numElems;++elementNumber)
    {
		if (mElemExist[elementNumber])
		{
			...
			for(int node=0;node<8;++node)
			{
				nodeId=mEdgesAtVoxel[8*elementNumber+node];
				displacement[3*node]=u[3*nodeId];
				displacement[3*node+1]=u[3*nodeId+1];
				displacement[3*node+2]=u[3*nodeId+2];
			}
...
//			std::cout<<" AnsysInput: "<<i<<". Element does not exist \n";
    ...
    std::cout<<"[NuTo::ConjugateGradientGridRed] AnsysInput: "<<numExistElem<<" elements created.\n";
	file<<"nsle,s \n nsel,inve \n ndele,all \n alls \n";
...
void NuTo::ConjugateGradientGridRed::CalculateNodesAtElement(int elementNumber,int* nodeNumbers)
{
	int rVoxelLocation[3];
	int residual1=mVoxelId[elementNumber]%((mGridDimension[0])*(mGridDimension[1]));
	...
	nodeNumbers[0] = mNodeId[rVoxelLocation[2]*(mGridDimension[0]+1)*(mGridDimension[1]+1)
	                        + rVoxelLocation[1]* (mGridDimension[1]+1) + rVoxelLocation[0]
	                                                                                    ];
	nodeNumbers[1] = nodeNumbers[0]+1;
	...
	nodeNumbers[7] = nodeNumbers[6]+1;


//	std::cout<<" nodesAtElement "<<elementNumber<<": "<<nodeNumbers[0]<<" "<<nodeNumbers[1]<<" "<<nodeNumbers[2]<<" "<<nodeNumbers[3]<<" "<<nodeNumbers[4]<<" "<<nodeNumbers[5]<<" "<<nodeNumbers[6]<<" "<<nodeNumbers[7]<<"\n";
}
