...
#ifdef ENABLE_EIGEN
	std::cout<<"Eigen vectors are used.\n";
	myType u;
...
//			CalculateStartGradientNodeByNodeII(&r[0]);
...
//			std::cout <<std::setw(width)<<" It.: "<< curIteration<< " norm gradient squared "<<squaredNorm<<std::endl;
			...
			std::cout << std::endl;
			std::cout << std::setw(width)<< "alpha * h " ;
			for (int count=0; count<mNumParameters; count++)
			{
				std::cout << std::setw(width)<< alpha* h[count] << "   " ;
			}
			std::cout << std::endl;
...
//				for(int nodeCol=0;nodeCol<8;++nodeCol)
//				{
//					nodeIdY =mNodeIds[8*elementNumber+nodeCol];
//					if (!mDofIsConstraint[3*nodeId])
//					{
////						std::cout<<" Stiffness part "<<mYoungsModulus[numStiff]*mBaseStiffness[3*node*dofsElem+3*nodeCol]<<" "<<mYoungsModulus[numStiff]*mBaseStiffness[(3*node+1)*dofsElem+3*nodeCol]<<" "<<mYoungsModulus[numStiff]*mBaseStiffness[(3*node+2)*dofsElem+3*nodeCol]<<"\n";
//						r[3*nodeId]+=mYoungsModulus[numStiff]*mBaseStiffness[3*node*dofsElem+3*nodeCol]*u[3*nodeIdY];
//						r[3*nodeId]+=mYoungsModulus[numStiff]*mBaseStiffness[(3*node)*dofsElem+3*nodeCol+1]*u[3*nodeIdY+1];
//						r[3*nodeId]+=mYoungsModulus[numStiff]*mBaseStiffness[(3*node)*dofsElem+3*nodeCol+2]*u[3*nodeIdY+2];
//					}
//					if (!mDofIsConstraint[3*nodeId+1])
//					{
//						r[3*nodeId+1]+=mYoungsModulus[numStiff]*mBaseStiffness[(3*node+1)*dofsElem+3*nodeCol]*u[3*nodeIdY];
//						r[3*nodeId+1]+=mYoungsModulus[numStiff]*mBaseStiffness[(3*node+1)*dofsElem+3*nodeCol+1]*u[3*nodeIdY+1];
//						r[3*nodeId+1]+=mYoungsModulus[numStiff]*mBaseStiffness[(3*node+1)*dofsElem+3*nodeCol+2]*u[3*nodeIdY+2];
//					}
//					if (!mDofIsConstraint[3*nodeId+2])
//					{
//						r[3*nodeId+2]+=mYoungsModulus[numStiff]*mBaseStiffness[(3*node+2)*dofsElem+3*nodeCol]*u[3*nodeIdY];
//						r[3*nodeId+2]+=mYoungsModulus[numStiff]*mBaseStiffness[(3*node+2)*dofsElem+3*nodeCol+1]*u[3*nodeIdY+1];
//						r[3*nodeId+2]+=mYoungsModulus[numStiff]*mBaseStiffness[(3*node+2)*dofsElem+3*nodeCol+2]*u[3*nodeIdY+2];
//
//					}
//				}
//						std::cout<<"  r["<<nodeId<<"] = "<<r[3*nodeId] <<" "<<r[3*nodeId+1] <<" "<<r[3*nodeId+2] <<"\n";
//			}
//			// Variants-Test:
...
void  NuTo::ConjugateGradientGrid::AnsysInput(int rNumParameters,boost::dynamic_bitset<>& elemExist, boost::dynamic_bitset<>& nodeExist,boost::dynamic_bitset<> &rDofIsConstraint,std::vector<double>& youngsModulus,int* rGridDimension,double* rVoxelSpacing,std::vector<int>& materialOfElem,std::vector<int>& allNodesAtVoxel,std::vector<double>& parameters)
{
   ...
   int countMat=youngsModulus.size();
    ...
    for (int i=0;i<(int) elemExist.size();++i)
    {
    	if(elemExist[i])
    	{
...
//    file<<"nsel,s,loc,x,0 \n";
//    file<<"d,all,all \n"; //eingespannt
//    file<<"nsel,s,loc,x,"<<(rGridDimension[0]+1)*rVoxelSpacing[0]<<" \n";
//    file<<"d,all,x,1 \n";
}
