...
//! @brief constructor
NuTo::LoadSurfaceBase3D::LoadSurfaceBase3D(int rLoadCase, StructureBase* rStructure, int rElementGroupId,
                                           int rNodeGroupId)
    : LoadBase(rLoadCase)
{
    // get element group
    ...
    for (auto itElement : *elementGroup)
    {
        ...
        catch (NuTo::MechanicsException& e)
        {
            ...
            ss << itElement.first;
            e.AddMessage("[NuTo::LoadSurfaceBase3D::LoadSurfaceBase3D] Error calculating surfaces for surface loads in "
                         "element " +
        ...
        catch (...)
        {
            ...
            assert(rStructure->ElementGetId(itElement.second) == itElement.first);
            ss << itElement.first;
            throw NuTo::MechanicsException("[NuTo::LoadSurfaceBase3D::LoadSurfaceBase3D] Error calculating surfaces "
...
void NuTo::LoadSurfaceBase3D::AddLoadToGlobalSubVectors(
        int rLoadCase, NuTo::FullVector<double, Eigen::Dynamic>& rActiceDofsLoadVector,
        ...
        for (int theIp = 0; theIp < integrationType->GetNumIntegrationPoints(); theIp++)
        {
            ...
            for (int iNode = 0; iNode < shapeFunctions.rows(); iNode++)
            {
                ...
                for (int iDispDof = 0; iDispDof < 3; iDispDof++)
                {
                    ...
                    {
                        // std::cout << "add to dof " << theDof << " " << shapeFunctions[iNode]*loadVector(countDispDof)
                        // << std::endl;
