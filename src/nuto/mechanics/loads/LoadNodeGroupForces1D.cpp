...
void NuTo::LoadNodeGroupForces1D::AddLoadToGlobalSubVectors(NuTo::FullMatrix<double>& rActiceDofsLoadVector, NuTo::FullMatrix<double>& rDependentDofsLoadVector)const
{
    ...
    for (Group<NodeBase>::const_iterator itNode=this->mGroup->begin(); itNode!=this->mGroup->end(); itNode++)
    {
        try
        {
            const NodeDisplacements<1>& node = dynamic_cast<const NodeDisplacements<1> &>(**itNode);
