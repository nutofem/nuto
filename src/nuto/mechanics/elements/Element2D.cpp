...
template void NuTo::Element2D::serialize(boost::archive::binary_iarchive & ar, const unsigned int version);
template void NuTo::Element2D::serialize(boost::archive::xml_iarchive & ar, const unsigned int version);
template void NuTo::Element2D::serialize(boost::archive::text_iarchive & ar, const unsigned int version);
template<class Archive>
void NuTo::Element2D::load(Archive & ar, const unsigned int version)
{
#ifdef DEBUG_SERIALIZATION
    std::cout << "start deserialize Element1D " << std::endl;
#endif
    ar & boost::serialization::make_nvp("ElementBase",boost::serialization::base_object<ElementBase >(*this));
...
template void NuTo::Element2D::serialize(boost::archive::binary_oarchive & ar, const unsigned int version);
template void NuTo::Element2D::serialize(boost::archive::xml_oarchive & ar, const unsigned int version);
template void NuTo::Element2D::serialize(boost::archive::text_oarchive & ar, const unsigned int version);
template<class Archive>
void NuTo::Element2D::save(Archive & ar, const unsigned int version)const
{
#ifdef DEBUG_SERIALIZATION
...
void NuTo::Element2D::SetNodePtrAfterSerialization(const std::map<std::uintptr_t, std::uintptr_t>& mNodeMapCast)
{
    for(std::vector<NodeBase*>::iterator it = mNodes.begin(); it != mNodes.end(); it++)
    {
        ...
        if (itCast!=mNodeMapCast.end())
        {
            *it = reinterpret_cast<NodeBase*>(itCast->second);
            ...
            throw MechanicsException("[NuTo::Element2D] The NodeBase-Pointer could not be updated.");
    }
}

BOOST_CLASS_EXPORT_IMPLEMENT(NuTo::Element2D)
