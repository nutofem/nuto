...
public:
    ...
    //! @param rIpDataType ... data type to decide what is stored at the integration point level
    //! @param rInterpolationType ... interpolation type
    ElementBase(const StructureBase* rStructure, ElementData::eElementDataType rElementDataType, int rNumIp,
                IpData::eIpDataType rIpDataType, const InterpolationType* rInterpolationType);
    ...
    virtual eError Evaluate(const ConstitutiveInputMap& rInput,
                            std::map<Element::eOutput, std::shared_ptr<ElementOutputBase>>& rOutput) = 0;

    template <int TDim>
    eError EvaluateConstitutiveLaw(const ConstitutiveInputMap& rConstitutiveInput,
                                   ConstitutiveOutputMap& rConstitutiveOutput, int rIP);
    ...
    //! @param rIp local Ip
    //! @param rNonlocalElement nonlocal element (must be in the range of the nonlocal element size stored at the
    //! element data level)
    ...
    virtual Eigen::VectorXd InterpolateDofGlobalCurrentConfiguration(int rTimeDerivative,
    ...
    virtual Eigen::Matrix<Eigen::VectorXd, Eigen::Dynamic, Eigen::Dynamic>
    InterpolateDofGlobalSurfaceDerivativeTotal(int rTimeDerivative, const Eigen::VectorXd& rParameter, int rDerivative,
                                               int rSurface) const
    {
        throw NuTo::MechanicsException(std::string("[") + __PRETTY_FUNCTION__ +
                                       "] Only implemented in ContinuumElementIGA.");
    ...
    virtual ContinuumContactElement<3, 2>& AsContinuumContactElement32()
    {
    ...
    //! @brief Computes all data in rVisualizationList for the visualization. Decomposes the element into small cells
    ...
    //! @brief Computes all data in rVisualizationList for the visualization. Extrapolates integration point data to
    //! element nodes
    //! @param rVisualize
    //! @param rVisualizationList: a list of visualization components to be visualized
    virtual void
    VisualizeExtrapolateToNodes(VisualizeUnstructuredGrid& rVisualize,
                                const std::list<std::shared_ptr<NuTo::VisualizeComponent>>& rVisualizationList);

    //! @brief Computes all data in rVisualizationList for the visualization. Visualizes integration point data as
    //! vertiex elements
    //! @param rVisualize
    //! @param rVisualizationList: a list of visualization components to be visualized
    virtual void
    VisualizeIntegrationPointData(VisualizeUnstructuredGrid& rVisualize,
                                  const std::list<std::shared_ptr<NuTo::VisualizeComponent>>& rVisualizationList);
...
protected:
    ...
    // the base class of the elements must not contain any data apart from a const pointer to the structure and a data
    // pointer
