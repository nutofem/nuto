...
template void NuTo::Element3D::serialize(boost::archive::binary_oarchive & ar, const unsigned int version);
template void NuTo::Element3D::serialize(boost::archive::xml_oarchive & ar, const unsigned int version);
template void NuTo::Element3D::serialize(boost::archive::text_oarchive & ar, const unsigned int version);
template<class Archive>
void NuTo::Element3D::save(Archive & ar, const unsigned int version)const
{
...
template void NuTo::Element3D::serialize(boost::archive::binary_iarchive & ar, const unsigned int version);
template void NuTo::Element3D::serialize(boost::archive::xml_iarchive & ar, const unsigned int version);
template void NuTo::Element3D::serialize(boost::archive::text_iarchive & ar, const unsigned int version);
template<class Archive>
void NuTo::Element3D::load(Archive & ar, const unsigned int version)
{
#ifdef DEBUG_SERIALIZATION
...
void NuTo::Element3D::SetNodePtrAfterSerialization(const std::map<std::uintptr_t, std::uintptr_t>& mNodeMapCast)
{
    for(std::vector<NodeBase*>::iterator it = mNodes.begin(); it != mNodes.end(); it++)
    {
        ...
        if (itCast!=mNodeMapCast.end())
        {
            *it = reinterpret_cast<NodeBase*>(itCast->second);
            ...
            throw MechanicsException("[NuTo::Element3D] The NodeBase-Pointer could not be updated.");
    }
}

BOOST_CLASS_EXPORT_IMPLEMENT(NuTo::Element3D)
