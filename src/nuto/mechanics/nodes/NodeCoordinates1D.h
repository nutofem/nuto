...
class NodeCoordinates1D : public virtual NodeBase
{
...
public:
    ...
    //! @brief constructor
    NodeCoordinates1D(const double rCoordinates[1]);

    //! @brief copy constructor
    NodeCoordinates1D(NodeCoordinates1D const& rOther)
    {
        (*this) = rOther;
    }

    //! @brief clones (copies) the node with all its data, it's supposed to be a new node, so be careful with ptr
    NodeCoordinates1D* Clone()const
    {
    	return new NodeCoordinates1D(*this);
    }

    //! @brief assignment operator
    void operator=(NodeCoordinates1D const& rOther)
    {
...
protected:
    double mCoordinates[1];

};
}//namespace NuTo
#ifdef ENABLE_SERIALIZATION
BOOST_CLASS_EXPORT_KEY(NuTo::NodeCoordinates1D)
#endif // ENABLE_SERIALIZATION
#endif //NODE_COORDINATES_1D_H
