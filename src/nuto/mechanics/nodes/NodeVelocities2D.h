...
#ifndef NODE_VELOCITIES_2D_H_
#define NODE_VELOCITIES_2D_H_
...
class NodeVelocities2D : public virtual NuTo::NodeBase
{
...
public:
    ...
    NodeVelocities2D(const double rVelocities[2]);
    ...
    void SetVelocities2D(const double rVelocities[2]);

    //! @brief writes the velocities of a node to the prescribed pointer
    //! @param rVelocities velocity
    void GetVelocities2D(double rVelocities[2])const;
...
#endif // ENABLE_SERIALIZATION
protected:
    double mVelocities[2]; //!< node velocity
};

}


#endif // NODE_VELOCITIES_2D_H_
