...
#ifndef NODE_VELOCITIES_1D_H_
#define NODE_VELOCITIES_1D_H_
...
class NodeVelocities1D : public virtual NuTo::NodeBase
{
...
public:
    ...
    NodeVelocities1D(const double rVelocities[1]);
    ...
    void SetVelocities1D(const double rVelocities[1]);

    //! @brief writes the velocities of a node to the prescribed pointer
    //! @param rVelocities velocity
    void GetVelocities1D(double rVelocities[1])const;
...
#endif // ENABLE_SERIALIZATION
protected:
    double mVelocities[1]; //!< node velocity
};

}


#endif // NODE_VELOCITIES_1D_H_
