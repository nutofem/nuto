...
class NodeGridDisplacements1D : public NodeGrid1D, public NodeDof<0,1,0,0,0>
{
...
public:
    ...
    //! @brief constructor
    NodeGridDisplacements1D(int rNodeId) : NodeGrid1D (rNodeId), NodeDof<0,1,0,0,0>()
    {}
    ...
    void serialize(Archive & ar, const unsigned int version)
    {
        ar & BOOST_SERIALIZATION_BASE_OBJECT_NVP(NodeGrid1D)
           & BOOST_SERIALIZATION_BASE_OBJECT_NVP(BOOST_IDENTITY_TYPE((NodeDof<0,1,0,0,0>)));
    }
#endif  // ENABLE_SERIALIZATION
    ...
    {
    	NodeDof<0,1,0,0,0>::SetGlobalDofs(rDOF);
    }
    ...
    {
    	NodeDof<0,1,0,0,0>::RenumberGlobalDofs(rMappingInitialToNewOrdering);
    }
