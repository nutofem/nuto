// $Id: $
#ifndef NodeRotations_H
#define NodeRotations_H

#include "nuto/math/FullMatrix.h"
#include "nuto/mechanics/nodes/NodeRotationsBase.h"
...
template<int NUMROTATIONS>
class NodeRotations : public  NodeRotationsBase
{
...
public:
    ...
    //! @brief constructor
    NodeRotations() : NodeRotationsBase ()
    {
    ...
    //! @brief constructor
    NodeRotations (const double rRotations[NUMROTATIONS])  : NodeRotationsBase ()
    {
        memcpy(mRotations,rRotations,NUMROTATIONS*sizeof(double));
    }
    ...
    //! @return global DOF
    int GetDofRotation(int rComponent)const
    {
        assert(rComponent>=0 && rComponent<NUMROTATIONS);
        return this->mDOF[rComponent];
    }

    //! @brief set the rotations
    //! @param rRotations  given rotations
    void SetRotations(const double rRotations[NUMROTATIONS])
    {
        memcpy(mRotations,rRotations,NUMROTATIONS*sizeof(double));
    }
    ...
    void GetRotations(double rRotations[NUMROTATIONS])const
    {
        memcpy(rRotations,mRotations,NUMROTATIONS*sizeof(double));
    }
...
protected:
    double mRotations[NUMROTATIONS];
    int mDOF[NUMROTATIONS];

};
}

#endif //NodeRotations_H
