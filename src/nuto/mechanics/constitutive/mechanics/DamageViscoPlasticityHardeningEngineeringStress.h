...
#ifndef DAMAGEVISCOPLASTICITYHARDENINGENGINEERINGSTRESS_H_
#define DAMAGEVISCOPLASTICITYHARDENINGENGINEERINGSTRESS_H_
...
class DamageViscoPlasticityHardeningEngineeringStress: public ConstitutiveBase
{
#ifdef ENABLE_SERIALIZATION
    friend class boost::serialization::access;
#endif // ENABLE_SERIALIZATION
public:
    DamageViscoPlasticityHardeningEngineeringStress();
...
protected:
    ...
    //! @param ... Viscosity Exponent \f$ \ViscosityExponent \f$
    ...
    //! @param ... Hardening value \f$ \Hardening value \f$
    double mHardening;

    //! @param ... Hardening exponent \f$ \Hardening exponent \f$
    double mHardeningExponent;
    ...
    //! @brief ... check whether hardening exponent is zero or positive
    //! @param rHardeningExponent ... hardening exponent
    void CheckHardeningExponent(double rHardeningExponent) const;
...
#define mTOLF 1.0e-12*this->mE						// define tolerance for Newton
    ...
    //! @brief ...  calculate hardening
    double CalculateHardening(const double rDeltaKappaInelastic, const double rPrevHardening,
    		const double rStrainRateNorm) const
    {
...
//    	RateFactor = 1.;
    	...
    	CurrentHardening += Plus(RateFactor*this->mHardening - rPrevHardening)*
    			this->mHardeningExponent*rDeltaKappaInelastic/
    			(1. + this->mHardeningExponent*rDeltaKappaInelastic);
...
//    	return this->mHardening*(1. - exp(-this->mHardeningExponent*rKappaInelastic));
    			...
    			// The stress rate is about 100 MPa/s

    //! @brief ...  calculate cumulative equivalent inelastic strain
    double CumulativeEquivalentStrainIncrement(EngineeringStrain3D rDeltaViscoPlasticStrain,
    		EngineeringStrain3D rDeltaPlasticStrain) const
    {
    	double DeltaKappaInelastic;
    	DeltaKappaInelastic = this->mDamageDistribution * pow(rDeltaViscoPlasticStrain.Norm(),2.);
    	DeltaKappaInelastic +=  (1. - this->mDamageDistribution) * pow(rDeltaPlasticStrain.Norm(),2.);
    	DeltaKappaInelastic = std::sqrt(DeltaKappaInelastic);
    	return DeltaKappaInelastic;
    }
    ...
    NuTo::FullVector<double,Eigen::Dynamic> Residual(
    		...
    		NuTo::FullVector<double,Eigen::Dynamic> rUnknown) const
		{
			...
			EngineeringStress3D Stress, ProjStress, OverStress;	// Stress and projection stress at the end of the time increment
    ...
    NuTo::FullMatrix<double,Eigen::Dynamic,Eigen::Dynamic> DResidualAn(
    		...
    		NuTo::FullVector<double,Eigen::Dynamic> rUnknown) const
		{
    		...
    		NuTo::FullMatrix<double,6,6> d2F_d2Sigma, d2F_d2Overstress;	// The second derivative of the flow surface by the stress
    		...
    		// calculate the overstress flow factor and its derivative
        	double ViscoFlowFactor;
        	NuTo::FullVector<double,6> ViscoFlowFactorDeriv;

        	ViscoFlowFactor = pow(OverStress.YieldSurfaceDruckerPrager3D(0., 0.)/mCompressiveStrength,mViscosityExponent);
        	OverStress.YieldSurfaceDruckerPrager3DDerivatives(ViscoFlowFactorDeriv,d2F_d2Overstress,0.);	// dJ2/dOverstress
        	ViscoFlowFactorDeriv *= mViscosityExponent/mCompressiveStrength;
        	ViscoFlowFactorDeriv *= pow(OverStress.YieldSurfaceDruckerPrager3D(0., 0.)/mCompressiveStrength,mViscosityExponent - 1.);
			...
			for (int j = 0; j < 6; j++) {
		    	...
		    	for (int i = 0; i < 6; i++) {
		    		deriv(i,j) = DELTA(i, j) *(1. + ViscoFlowFactor*rDeltaTime*Sgn(rVviscoP)/rViscosity) +
		    				(rDeltaTime*Sgn(rVviscoP)/rViscosity)*OverStress[i]*ViscoFlowFactorDeriv[j] +
			...
			for (int j = 6; j < 12; j++) {
		    	for (int i = 0; i < 6; i++) {
		    		deriv(i,j) = - DELTA(i, j-6) * ViscoFlowFactor * rDeltaTime*Sgn(rVviscoP)/rViscosity -
		    				(rDeltaTime*Sgn(rVviscoP)/rViscosity)*OverStress[i]*ViscoFlowFactorDeriv[j-6];
				}
			}
...
#ifdef ENABLE_SERIALIZATION
BOOST_CLASS_EXPORT_KEY(NuTo::DamageViscoPlasticityHardeningEngineeringStress)
#endif //ENABLE_SERIALIZATION

#endif // DamageViscoPlasticityHardeningngineeringStress_H_
