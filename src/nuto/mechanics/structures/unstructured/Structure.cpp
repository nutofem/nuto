...
void NuTo::Structure::SaveUpdate(const std::string& filename, std::string rType) const
{
    ...
    {
        std::string s(std::string("[NuTo::Structure::SaveUpdate]File save exception in boost - ") +
                      std::string(e.what()));
        throw MechanicsException(s);
    }
    catch (MechanicsException& e)
...
void NuTo::Structure::loadImplement(Archive& ar)
{
    ...
    {
        mNodeMapOldNewPtr.insert(std::pair<std::uintptr_t, std::uintptr_t>(
                itCastNode->second, reinterpret_cast<std::uintptr_t>(it->second)));
    }
    ...
    for (boost::ptr_map<int, ElementBase>::iterator it = mElementMap.begin(); it != mElementMap.end();
         it++, itCastElement++)
    {
        mElementMapOldNewPtr.insert(std::pair<std::uintptr_t, std::uintptr_t>(
                itCastElement->second, reinterpret_cast<std::uintptr_t>(it->second)));
    }
        ...
        for (int i = 0; i < size; i++)
        {
            ...
            if (itCast != mElementMapOldNewPtr.end())
            {
                ...
                throw MechanicsException(
                        "[NuTo::Structure::loadImplement] The ElementBase-Pointer could not be updated.");
        }

        it->assign(reinterpret_cast<ElementBase**>(&mMISAddress[0]),
                   reinterpret_cast<ElementBase**>(&mMISAddress[size]));
    }
#endif

    // update the node ptr in elements
    for (boost::ptr_map<int, ElementBase>::iterator itElements = mElementMap.begin(); itElements != mElementMap.end();
         itElements++)
    {
    ...
    for (boost::ptr_map<int, ConstraintBase>::iterator itConstraints = mConstraintMap.begin();
         itConstraints != mConstraintMap.end(); itConstraints++)
    {
...
void NuTo::Structure::Restore(const std::string& filename, std::string rType)
{
    ...
    try
    {
        ...
        if (rType == "BINARY")
        {
            ...
            {
                throw MechanicsException("[NuTo::Structure::Restore] Data type of object in file (" + typeIdString +
                                         ") is not identical to data type of object to read (" + this->GetTypeId() +
                                         ").");
            }
            oba& boost::serialization::make_nvp(typeIdString.c_str(), *this);
    ...
    {
        std::string s(std::string("[NuTo::Structure::Restore] File save exception in boost - ") +
                      std::string(e.what()));
        throw MechanicsException(s);
    }
    catch (MechanicsException& e)
...
void NuTo::Structure::RestoreUpdate(const std::string& filename, std::string rType)
{
    ...
    {
        std::string s(std::string("[NuTo::Structure::RestoreUpdate] File save exception in boost - ") +
                      std::string(e.what()));
        throw MechanicsException(s);
    }
    catch (MechanicsException& e)
...
#pragma omp parallel shared(rStructureOutput) // firstprivate(elementOutputMap)
            {
                ...
                elementOutputMap[Element::eOutput::GLOBAL_ROW_DOF] =
                        std::make_shared<ElementOutputBlockVectorInt>(mDofStatus);
            ...
            ElementBase* elementPtr = elementIter->second;
                        ...
                        if (error != eError::SUCCESSFUL)
                        {
                            ...
                            else if (errorGlobal != error)
                            {
                                throw MechanicsException(
                                        __PRETTY_FUNCTION__,
                                        "elements have returned multiple different error codes, can't handle that.");
                            }
                        }
...
void NuTo::Structure::BuildNonlocalData(const ConstitutiveBase* rConstitutive)
{
    ...
    for (unsigned int theIp = 0; theIp < indexIp.size(); theIp++)
    {
        ...
        for (unsigned int theNeighborPoint = 0; theNeighborPoint < numNeighborPoints; theNeighborPoint++)
        {
            ...
            elementPtr->SetNonlocalWeight(localIpNumber, indexElement[theNeighborIndex], indexIp[theNeighborIndex],
                                          indexIpVolume[theNeighborIndex] * dists[theNeighborPoint] * totalVolume);
        }
    }
    ...
    for (elementIter = this->mElementMap.begin(); elementIter != this->mElementMap.end(); elementIter++)
    {
        ...
        for (int theIp = 0; theIp < elementPtr->GetNumIntegrationPoints(); theIp++)
        {
            ...
            double sumW(0.);
            for (int countNonlocalElement = 0; countNonlocalElement < (int)nonlocalElements.size();
                 countNonlocalElement++)
            {
...
//! @brief import from gmsh, creates groups according to gmsh's physical entities and creates an interpolation types for
//! each group
...
NuTo::FullMatrix<int, Eigen::Dynamic, Eigen::Dynamic>
NuTo::Structure::ImportFromGmsh(const std::string& rFileName, ElementData::eElementDataType rElementData,
...
NuTo::FullMatrix<int, Eigen::Dynamic, Eigen::Dynamic> NuTo::Structure::ImportFromGmsh(const std::string& rFileName,
                                                                                      const std::string& rElementData,
                                                                                      const std::string& rIPData)
{
    Timer timer(__FUNCTION__, GetShowTime(), GetLogger());
...
NuTo::FullMatrix<int, Eigen::Dynamic, Eigen::Dynamic>
NuTo::Structure::ImportFromGmshAux(const std::string& rFileName, ElementData::eElementDataType rElementData,
                                   IpData::eIpDataType rIPData)
{
    const unsigned int num_elm_nodes[24] = {0,  2,  3,  4, 4, 8,  6,  5,  3, 6,  9, 10,
                                            27, 18, 14, 1, 8, 20, 15, 13, 0, 10, 0, 15};
    ...
    bool match(phrase_parse(itFirst, line.end(),
                            uint_[ref(mayor_version) = qi::_1] >> '.' >> uint_[ref(minor_version) = qi::_1] >>
                                    uint_[ref(binary) = qi::_1] >> uint_[ref(double_size) = qi::_1],
                            ascii::space));
    ...
    // std::cout << "version " << mayor_version <<"," << minor_version  << " binary " << binary << " double_size " <<
    ...
    if (binary == 0) // read ASCII - file
    {
        ...
        for (unsigned int node_count = 0; node_count < num_nodes; node_count++)
        {
            getline(file, line);
            itFirst = line.begin();
            match = phrase_parse(itFirst, line.end(), uint_[ref(nodes[node_count].id) = qi::_1] >>
        ...
        for (unsigned int element_count = 0; element_count < num_elements; element_count++)
        {
            ...
            getline(file, line);
            itFirst = line.begin();
            match = phrase_parse(itFirst, line.end(), *(uint_[push_back(boost::phoenix::ref(tmp_elem_data), qi::_1)]),
                                 ascii::space);
    ...
    for (unsigned int elementCount = 0; elementCount < elements.size(); elementCount++)
    {
        ...
        //    	case 93: //	125-node fourth order hexahedron (8 nodes associated with the vertices, 36 with the
        //    edges,
        ...
        int groupId = elements[elementCount].tags[0]; // NuTo groupId == gmsh groupId. // This might cause errors if
        // groups exist before the gmsh import.
        ...
        else
        {
            ...
            for (int id : itGroupInterpolation->second)
            {
                ...
                Interpolation::eTypeOrder groupTypeOrder =
                        interpolationType.Get(Node::eDof::COORDINATES).GetTypeOrder();
    ...
    NuTo::FullMatrix<int, Eigen::Dynamic, Eigen::Dynamic> ids(
            numGroups, numMaxInterpolationTypesPerGroup + 1); // +1 since first column is group id
...
NuTo::FullMatrix<int, Eigen::Dynamic, Eigen::Dynamic> NuTo::Structure::ImportFromGmshTemp(const std::string& rFileName,
                                                                                          int elementType)
{
    const unsigned int num_elm_nodes[24] = {0,  2,  3,  4, 4, 8,  6,  5,  3, 6,  9, 10,
                                            27, 18, 14, 1, 8, 20, 15, 13, 0, 10, 0, 15};
    ...
    ids(0, 0) = GroupCreate(NuTo::eGroupId::Nodes);
    ...
    for (unsigned int elementCount = 0; elementCount < elements.size(); elementCount++)
    {
        ...
        if (elementType == elements[elementCount].type && nodeNumbers.rows() > 0)
        {
            for (int i = 0; i < nodeNumbers.rows(); i++)
            {
                std::vector<int>::iterator it = find(nodeids.begin(), nodeids.end(), nodeNumbers(i));
                if (it == nodeids.end())
                {
                    nodeids.push_back(nodeNumbers(i));
                }
            }
        }
    }
