...
public:
    ...
    //! @brief Adds the delta rhs of the constrain equation whose RHS is incrementally increased in each load step /
    //! time step
    //! @param rTimeDependentConstraint ... constraint, whose rhs is increased as a function of time
    //! @param mTimeDependentConstraintFactor ... first row time, rhs of the constraint (linear interpolation in between
    //! afterwards linear extrapolation)
    void AddTimeDependentConstraint(
            int rTimeDependentConstraint,
            const NuTo::FullMatrix<double, Eigen::Dynamic, Eigen::Dynamic>& mTimeDependentConstraintFactor);

    //! @brief Adds the delta rhs of the constrain equation whose RHS is incrementally increased in each load step /
    //! time step
    //! @param rTimeDependentConstraint ... constraint, whose rhs is increased as a function of time
    //! @param rTimeDependentConstraintFunction ... function that calculates the time dependent constraint factor for
    //! the current time step
    void
    AddTimeDependentConstraintFunction(int rTimeDependentConstraint,
                                       const std::function<double(double rTime)>& rTimeDependentConstraintFunction);
    ...
    void SetTimeDependentLoadCase(int rLoadCase,
                                  const NuTo::FullMatrix<double, Eigen::Dynamic, Eigen::Dynamic>& rLoadRHSFactor);
    ...
    //! @return id of the result, so that it could be modified afterwards
    int AddResultElementGroupIpData(const std::string& rResultStr, int rElementGroupId, int rComponent,
                                    const std::vector<int>& rIP, NuTo::IpData::eIpStaticDataType rIpDataType);
    ...
    void SetWriteForced(bool value)
    {
        mWriteForced = value;
    }
...
// empty private construct required for serialization
...
#ifdef ENABLE_SERIALIZATION
    TimeIntegrationBase()
        : mLoadVectorStatic(DofStatus())
        , mLoadVectorTimeDependent(DofStatus()){};
#endif // ENABLE_SERIALIZATION
    ...
    int mTimeStepVTK;
    // if the time between the current time step and the previous plotted step is larger than mMaxDeltaTimeStepPlot a
    // vtk plot is performed
    ...
    // vector of groups of nodes for which the residual (corresponding to the reaction forces induced by constraints) is
    // given as output
