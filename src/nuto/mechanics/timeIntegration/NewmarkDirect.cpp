...
NuTo::eError NuTo::NewmarkDirect::Solve(double rTimeDelta)
{
    ...
    try
    {
        ...
        if (mToleranceResidual < residual_mod.CalculateInfNorm())
        {
            ...
            //            throw MechanicsException(__PRETTY_FUNCTION__, "Initial configuration is not in (dynamic)
            //            equilibrium.");
        }
        ...
        while (curTime < rTimeDelta)
        {
            ...
            //! everything seems
            //! to work fine, but valgrind tells you otherwise. Problem is, that the DofType is changed during the
            ...
            for (const auto& activeDofs : mStepActiveDofs)
            {
                ...
                while (!(normResidual < mToleranceResidual) && iteration < mMaxNumIterations)
                {
                    ...
                    // ******************************************************
                    delta_dof_dt0.J =
                            BuildHessianModAndSolveSystem(hessian0, hessian1, hessian2, residual_mod, timeStep);
                    ...
                    if (mAutomaticTimeStepping)
                    {
                        ...
                        if (timeStep < mMinTimeStep)
                        {
                            ...
                            throw MechanicsException(
                                    "[NuTo::NewmarkDirect::Solve] no convergence, the current time step is too short.");
                        }
                    }
                    else
                    {
                        throw MechanicsException("[NuTo::NewmarkDirect::Solve] no convergence with the current maximum "
                                                 "number of iterations, either use automatic time stepping, reduce the "
                                                 "time step or the minimal line search cut back factor.");
                    }
                }
            }
        }
