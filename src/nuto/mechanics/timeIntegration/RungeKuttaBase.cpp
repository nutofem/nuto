...
NuTo::Error::eError NuTo::RungeKuttaBase::Solve(StructureBase& rStructure, double rTimeDelta)
{
#ifdef SHOW_TIME
...
//to be implemented rStructure.SetCurrentTime(mTime);
    ...
    if (mShowTime)
        rStructure.GetLogger()<<"[NuTo::RungeKuttaBase::Solve] " << difftime(end,start)/CLOCKS_PER_SEC << "sec(" << wend-wstart <<")\n";
#else
    if (mShowTime)
        rStructure.GetLogger()<< "[NuTo::RungeKuttaBase::Solve] " << difftime(end,start)/CLOCKS_PER_SEC << "sec" << "\n";
#endif
#endif
