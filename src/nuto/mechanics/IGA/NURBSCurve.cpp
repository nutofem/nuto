...
NuTo::NURBSCurve::NURBSCurve(const Eigen::VectorXd& rKnots, const Eigen::MatrixXd& rControlPoints,
...
NuTo::NURBSCurve::NURBSCurve(int rDegree, const Eigen::MatrixXd& rPoints, Eigen::MatrixXd& A,
...
Eigen::VectorXi NuTo::NURBSCurve::GetParameterControlPoints(double rParameter)
{
    ...
    for (int j = 0; j <= mDegree; j++)
        controlPointIDs(j) = spanIdx - mDegree + j;
...
const Eigen::MatrixXd& NuTo::NURBSCurve::GetBezierExtraction(int rElementID) const
{
    ...
    if (mBezierOperators.size() == 0)
        throw Exception("[NURBSCurve] The Bézier extraction has to be calculated first. Use the function "
                        "'BezierElementExtractionOperators'.");
...
{
    int spanIdx = ShapeFunctionsIGA::FindSpan(rParameter, mDegree, mKnots);
    Eigen::VectorXd basisFunctions = ShapeFunctionsIGA::BasisFunctionsAndDerivativesRat(
            rDerivative, rParameter, spanIdx, mDegree, mKnots, mWeights);
...
void NuTo::NURBSCurve::InsertKnot(double rKnotToInsert, int rMultiplicity)
{
    ...
    for (int i = k - initialMultiplicity; i < GetNumControlPoints(); i++)
        newControlPoints.row(i + rMultiplicity) = controlPointsProjected.row(i);
    for (int i = 0; i <= mDegree - initialMultiplicity; i++)
        Rw.row(i) = controlPointsProjected.row(k - mDegree + i);
                                           ...
                                           double& rParameterStartMaster) const
{
    ...
    int dim = rCoordinatesSlave.rows();

    if ((rCoordinatesSlave - mControlPoints.block(GetNumControlPoints() - 1, 0, 1, dim).transpose()).norm() < 1.e-10)
        rParameterStartMaster = 1.;
    if ((rCoordinatesSlave - mControlPoints.block(0, 0, 1, dim).transpose()).norm() < 1.e-10)
...
NuTo::NURBSCurve::buildIGAStructure(NuTo::Structure& rStructure, const std::set<NuTo::Node::eDof>& rSetOfDOFS,
                                    ...
                                    Eigen::VectorXi& nodeIDs) const
{
    ...
    for (auto& it : rSetOfDOFS)
        rStructure.InterpolationTypeAdd(rNewInterpolation, it, NuTo::Interpolation::eTypeOrder::SPLINE, degree,
                                        vecKnots, mWeights);
