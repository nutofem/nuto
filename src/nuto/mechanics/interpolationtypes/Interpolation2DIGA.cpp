...
Eigen::MatrixXd
NuTo::Interpolation2DIGA::CalculateDerivativeShapeFunctionsNatural(const Eigen::VectorXd& rCoordinates,
...
Eigen::MatrixXd
NuTo::Interpolation2DIGA::CalculateDerivativeShapeFunctionsNatural(int rIP, const Eigen::VectorXi& rKnotIDs) const
{
    assert(!mUpdateRequired);
    assert(rIP >= 0 && rIP < mIPCoordinates.rows());
...
Eigen::MatrixXd NuTo::Interpolation2DIGA::CalculateMatrixN(const Eigen::VectorXd& rCoordinates,
                                                           const Eigen::VectorXi& rKnotIDs) const
{
    assert(rKnotIDs.rows() == 2);
    assert(rKnotIDs(0) < mKnotsX.rows());
    ...
    auto shapeFunctions = ShapeFunctionsIGA::BasisFunctionsAndDerivatives2DRat(0, parameter, rKnotIDs, mDegree, mKnotsX,
                                                                               mKnotsY, mWeights);
    assert(shapeFunctions.rows() == (mDegree(0) + 1) * (mDegree(1) + 1));
...
Eigen::VectorXd
NuTo::Interpolation2DIGA::CalculateNaturalSurfaceCoordinates(const Eigen::VectorXd& rNaturalSurfaceCoordinates,
                                                             int rSurface, const Eigen::MatrixXd& rKnots) const
{
    switch (rSurface)
    {
    case -1:
        return Eigen::Vector2d(transformation(rNaturalSurfaceCoordinates(0), rKnots(0, 0), rKnots(0, 1)),
                               transformation(rNaturalSurfaceCoordinates(1), rKnots(1, 0), rKnots(1, 1)));
    case 0:
    ...
    default:
        throw MechanicsException(__PRETTY_FUNCTION__, "IGA2D has exactly four surfaces, rSurface = 0,1,2,3, and its "
                                                      "whole 2D domain (rSurface = -1). You tried to access " +
