                             ...
                             NuTo::SparseMatrixCSRVector2<T>(rNumRows_)
{
    if (rNumRows_!=rNumColumns_)
        throw MathException("[NuTo::SparseMatrixCSRVector2Symmetric<T>::SparseMatrixCSRVector2Symmetric] Symmetric matrix must have same number of rows and columns.");
}
                             ...
                             NuTo::SparseMatrixCSRVector2<T>(rFullMatrix.GetNumRows())
{
	...
	for (int row = 0; row < rFullMatrix.GetNumRows(); row++)
	{
		for (int col = row; col < rFullMatrix.GetNumColumns(); col++)
		{
			...
			{
			    throw MathException("[NuTo::SparseMatrixCSRVector2Symmetric<T>::SparseMatrixCSRVector2Symmetric] Full matrix is not symmetric.");
			}
		}
	}
}
...
void NuTo::SparseMatrixCSRVector2Symmetric<T>::AddValue(int rRow, int rColumn, const T& rValue)
{
	...
	{
		throw MathException("[SparseMatrixCSRVector2Symmetric::AddValue] row index is out of bounds.");
	}
	if (rColumn >= (int)this->mValues.size() || rColumn<0)
	{
		throw MathException("[SparseMatrixCSRVector2Symmetric::AddValue] column index is out of bounds.");
	}
    if (rColumn < rRow)
    {
        std::cout << "col " << rColumn << " row " << rRow << std::endl;
    	throw MathException("[SparseMatrixCSRVector2Symmetric::AddValue] upper triangle is stored for symmetric matrices.");
    }
...
NuTo::SparseMatrixCSRVector2Symmetric<T> NuTo::SparseMatrixCSRVector2Symmetric<T>::operator+ ( const NuTo::SparseMatrixCSRVector2Symmetric<T> &rOther )
{
	...
	{
		throw MathException("[SparseMatrixCSRVector2Symmetric::operator+] both matrices must have zero based indexing.");
	}
	SparseMatrixCSRVector2Symmetric<T> result(this->GetNumRows(), this->GetNumColumns());
	for (int row = 0; row < this->GetNumRows(); row++)
	{
		...
		while ((thisPos < thisColumnVec.size()) || (otherPos < otherColumnVec.size()))
		{
			...
			{
				// no additional entries in this row (set column to an invalid value)
				otherColumn = this->GetNumColumns();
			}
...
template<class T>
NuTo::SparseMatrixCSRVector2Symmetric<T> NuTo::SparseMatrixCSRVector2Symmetric<T>::operator* ( const T &rOther ) const
{
	SparseMatrixCSRVector2Symmetric<T> result(*this);
...
{
	throw MathException("[SparseMatrixCSRVector2Symmetric::ReorderColumns] To be implemented.");
...
void NuTo::SparseMatrixCSRVector2Symmetric<T>::Resize(int rNumRows, int rNumColumns)
{
    ...
    if (rNumRows!=rNumColumns)
		throw MathException("[SparseMatrixCSRVector2Symmetric::Resize] number of rows and column has to be identical for symmetric matrices.");
