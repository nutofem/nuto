...
template<class ...Args>
void NuTo::SerializeStreamIn::SerializeData(Args&... rValue)
{
    ReadData(rValue...);
}

void NuTo::SerializeStreamIn::ReadData(double &rValue)
{
    if (mIsBinary)
    {
        mFileStream.read(reinterpret_cast<char*>(&rValue), sizeof(double));
    }
    else
    {
        std::string line;
        ...
        rValue = std::stod(line);
    }
}

template<typename T, int TRows, int TCols, int TOptions, int TMaxRows, int TMaxCols>
void NuTo::SerializeStreamIn::ReadData(Eigen::Matrix<T, TRows, TCols, TOptions, TMaxRows, TMaxCols> & rMatrix)
{
