...
namespace BenchmarkInternal
{
    ...
    struct Result
    {
        Result(std::string rTestCase, std::string rTestName, std::vector<double> rSeconds)
                : mTestCase(rTestCase), mTestName(rTestName)
        {
    ...
    public:
        void AddTestResult(Result rTestResult)
        {
            ...
            std::cout << "Test " << std::setw(40) << std::left << name
                      << " took " << rTestResult.mTimeTotal << "s" << std::endl;
        }
        void Report()
        {
            WriteHeader();
    ...
    protected:
        virtual void WriteTestResult(const Result& rT) override
        {
            mFile << rT.mTestCase << "::" << rT.mTestName << ','
                  << rT.mTimeMean << std::endl;
        }
    private:
        std::ofstream mFile;
    };
    ...
    protected:
        ...
        {
            mFile << "    <testcase name=\"" << rT.mTestCase << "::" << rT.mTestName
                  << "\" time=\"" << rT.mTimeMean << "\" \\>\n";
        }
...
#define BENCHMARK(TestCase, TestName, RunnerName) \
    BENCHMARK_HELPER(CLASS_NAME(TestCase, TestName), \
                     INSTANCE_NAME(TestName, __LINE__), \
                     RunnerName, \
                     TestCase, TestName)
...
public: \
    NAME() \
    {\
        BenchmarkInternal::Holder::GetInstance().AddBenchmark(BenchmarkInternal::Benchmark({#TestCase, #TestName,&Test})); \
    }\
}; \
NAME INSTANCE; \
